...
export default function run(argv) {
  // The interactive flow needs a terminal for its two confirmations. Without
  // one, readline's question callback never fires: the process would fall out
  // of the event loop and exit 0 having written nothing — a silent no-op that
  // reads as success. Agent shells, scripts, and CI all land here, so fail
  // loudly and name the flag that works instead.
  const flags = parseFlags(argv);
  if (!flags.yes && !process.stdin.isTTY) {
    process.stderr.write(
      'honestweek init: stdin is not a terminal, so the two setup confirmations cannot be asked.\n' +
