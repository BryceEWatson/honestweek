...
import {
  HEX_64, addWeeks, assessCandidatePublicSafety, effectiveAutomaticCarryWeeks, effectiveRetentionWeeks, exactObject,
  retiredRow, subjectFingerprint, validateLifecycleWeek, validateRetired, validDateKey, validIso,
...
function exactWeekOffset(first, later) {
  const delta = new Date(`${later}T00:00:00.000Z`).getTime() - new Date(`${first}T00:00:00.000Z`).getTime();
  return Number.isInteger(delta / (7 * 24 * 60 * 60 * 1000)) && delta >= 0;
}

function currentlySafeText(value, config) {
  return typeof value === 'string' && createRedactor(config).redact(value) === value &&
    assessPublicRendition(value, config) === 'low';
}

function assertCarryCurrentPrivacy(value, config) {
  for (const record of value.weeks) {
    for (const entry of record.entries) {
      if (assessCandidatePublicSafety(entry.candidate, config).decision !== 'automatic-safe' ||
          (entry.candidate.project !== null && createRedactor(config).redact(entry.candidate.project) !== entry.candidate.project)) {
        throw new Error('carry contains text that is unsafe under the current privacy configuration.');
      }
    }
    for (const retired of record.retired) {
      if (retired.subject !== null && !currentlySafeText(retired.subject, config)) {
        throw new Error('carry contains retired text that is unsafe under the current privacy configuration.');
      }
    }
  }
}

function sanitizedPriorRecord(record, config) {
  const copy = structuredClone(record);
  copy.entries = copy.entries.filter((entry) =>
    assessCandidatePublicSafety(entry.candidate, config).decision === 'automatic-safe' &&
    (entry.candidate.project === null || createRedactor(config).redact(entry.candidate.project) === entry.candidate.project));
  for (const retired of copy.retired) {
    if (retired.subject !== null && !currentlySafeText(retired.subject, config)) {
      retired.subject = null;
      retired.subjectFingerprint = null;
    }
  }
  return copy;
}

export function validateCarryEntry(entry, config, recordWeekStart = null) {
  ...
  if (!HEX_64.test(entry.lineageRef) || !HEX_64.test(entry.itemRef) ||
      ...
      !exactWeekOffset(entry.firstSeenWeek, entry.lastShownWeek) ||
      ...
      (entry.manualTargetWeek !== null &&
       (!exactWeekOffset(entry.firstSeenWeek, entry.manualTargetWeek) || entry.manualTargetWeek <= entry.firstSeenWeek)) ||
  ...
  if (entry.automaticThroughWeek !== null &&
      ((entry.category !== 'nextSteps' && !entry.candidate?.discriminator?.startsWith('unresolved-idea:')) ||
       ![addWeeks(entry.firstSeenWeek, 1), addWeeks(entry.firstSeenWeek, 2)].includes(entry.automaticThroughWeek))) {
    throw new Error('carry automatic window exceeds the owner-approved bound.');
  }
  if (recordWeekStart !== null &&
      (entry.firstSeenWeek > recordWeekStart || !exactWeekOffset(entry.firstSeenWeek, recordWeekStart) ||
       entry.lastShownWeek > recordWeekStart ||
       (entry.manualTargetWeek !== null && entry.manualTargetWeek > recordWeekStart &&
        entry.manualTargetWeek !== addWeeks(recordWeekStart, 1)))) {
    throw new Error('carry entry is not bounded to its canonical week record.');
  }
  ...
  validateDigestCandidate(entry.candidate, config, digestPolicyForConfig(config), { historical: true });
  return entry;
}
...
function validateAdjacentTransition({ previous, current, retired, terminalLineages, weekStart }) {
  const retiredByLineage = new Map();
  for (const row of retired) {
    const rows = retiredByLineage.get(row.lineageRef) ?? [];
    rows.push(row);
    retiredByLineage.set(row.lineageRef, rows);
    const active = current.get(row.lineageRef);
    if (active && row.category !== active.category) throw new Error('carry retirement changes lineage category.');
    if (active && row.reason !== 'superseded') {
      throw new Error('carry lineage is active and terminally retired in the same record.');
    }
  }
  for (const [lineageRef, rows] of retiredByLineage) {
    const superseded = rows.filter((row) => row.reason === 'superseded');
    if (superseded.length > 1) throw new Error('carry lineage has multiple superseded retirements in one record.');
    if (superseded.length) {
      const active = current.get(lineageRef);
      if (!active || active.itemRef === superseded[0].itemRef) {
        throw new Error('carry superseded retirement lacks one distinct active replacement.');
      }
    }
  }
  for (const entry of current.values()) {
    if (terminalLineages.has(entry.lineageRef)) throw new Error('carry lineage resumes after retirement.');
    const priorEntry = previous?.get(entry.lineageRef);
    if (priorEntry) {
      if (entry.category !== priorEntry.category || entry.firstSeenWeek !== priorEntry.firstSeenWeek ||
          entry.automaticThroughWeek !== priorEntry.automaticThroughWeek ||
          entry.lastShownWeek < priorEntry.lastShownWeek || entry.strength !== priorEntry.strength) {
        throw new Error('carry lineage continuity is invalid.');
      }
    } else if (previous !== null && entry.firstSeenWeek !== weekStart) {
      throw new Error('carry lineage starts outside its first retained record.');
    }
  }
  if (previous !== null) {
    for (const [lineageRef, priorEntry] of previous) {
      const active = current.get(lineageRef);
      const rows = retiredByLineage.get(lineageRef) ?? [];
      const superseded = rows.filter((row) => row.reason === 'superseded');
      const terminal = rows.filter((row) => row.reason !== 'superseded');
      if (active && active.itemRef !== priorEntry.itemRef) {
        if (superseded.length !== 1 || superseded[0].itemRef !== priorEntry.itemRef) {
          throw new Error('carry item replacement lacks its superseded retirement.');
        }
      } else if (superseded.length) {
        throw new Error('carry has a superseded retirement without an item replacement.');
      }
      if (!active && terminal.length !== 1) throw new Error('carry lineage disappearance lacks one retirement.');
    }
    for (const row of retired) {
      if (!previous.has(row.lineageRef) &&
          (row.reason !== 'privacy-withheld' || row.subject !== null || row.subjectFingerprint !== null)) {
        throw new Error('carry retirement has no safe immediately prior active lineage.');
      }
    }
  }
  for (const row of retired) if (row.reason !== 'superseded') terminalLineages.add(row.lineageRef);
}

export function validateCarry(value, config, { prior = false } = {}) {
  ...
  if (value.version !== 1 || !Array.isArray(value.weeks) || !Array.isArray(value.tombstones) ||
      value.weeks.length > (prior ? 12 : effectiveRetentionWeeks(config))) throw new Error('carry schema is invalid.');
  let priorWeekStart = null;
  const terminalLineages = new Set();
  let previousActive = null;
  for (const record of value.weeks) {
    exactObject(record, WEEK_RECORD_KEYS, 'carry week record');
    validateLifecycleWeek(record.week, 'carry week');
    if (priorWeekStart && priorWeekStart >= record.week.start) throw new Error('carry weeks are duplicate or not canonical.');
    priorWeekStart = record.week.start;
    if (!Array.isArray(record.entries) || !Array.isArray(record.retired)) throw new Error('carry week record is invalid.');
    const refs = new Set();
    for (const entry of record.entries) {
      validateCarryEntry(entry, config, record.week.start);
    ...
    for (const retired of record.retired) {
      validateRetired(retired, config, { historical: true });
      const key = `${retired.lineageRef}\0${retired.itemRef}`;
      if (retiredRefs.has(key)) throw new Error('carry week has duplicate retired lineage/item.');
      retiredRefs.add(key);
    }
    ...
    const currentActive = new Map(record.entries.map((entry) => [entry.lineageRef, entry]));
    validateAdjacentTransition({
      previous:previousActive, current:currentActive, retired:record.retired,
      terminalLineages, weekStart:record.week.start,
    });
  ...
  if (!prior) assertCarryCurrentPrivacy(value, config);
  return value;
}
...
export function readCarry(cwd, config, { optional = false } = {}) {
  const loaded = readCanonicalState(cwd, CARRY_STORE, optional, (value) => validateCarry(value, config, { prior: true }));
  if (!loaded) return { value: null, hash: null, bytes: null };
  const { value, bytes } = loaded;
...
export function writeCarry(cwd, value, config, fs) {
  ...
  if (Buffer.byteLength(text) > MAX_STATE_BYTES) throw new Error(`${CARRY_STORE} exceeds the 8 MiB cap.`);
...
export function carryTombstonesForWeek(carry, _week) {
  return carry?.tombstones ?? [];
}
...
export function readCarryPending(cwd, config, { optional = false } = {}) {
  return readCanonicalState(cwd, CARRY_PENDING, optional, (value) => validateCarryPending(value, config))?.value ?? null;
}

function readCanonicalState(cwd, filename, optional, validate) {
  const path = join(cwd, filename);
  if (!existsSync(path)) {
    if (optional) return null;
    throw new Error(`${filename} not found.`);
  }
  if (statSync(path).size > MAX_STATE_BYTES) throw new Error(`${filename} exceeds the 8 MiB cap.`);
  const bytes = readFileSync(path);
  if (bytes.length > MAX_STATE_BYTES) throw new Error(`${filename} exceeds the 8 MiB cap.`);
  let value;
  try { value = JSON.parse(bytes.toString('utf8')); }
  catch (error) { throw new Error(`${filename} is not valid JSON (${error.message}).`); }
  validate(value);
  if (canonicalJson(value) !== bytes.toString('utf8')) throw new Error(`${filename} is not canonical JSON.`);
  return { value, bytes };
}

export function writeCarryPending(cwd, value, config, fs) {
  validateCarryPending(value, config);
  if (Buffer.byteLength(canonicalJson(value)) > MAX_STATE_BYTES) throw new Error(`${CARRY_PENDING} exceeds the 8 MiB cap.`);
  atomicWriteJson(join(cwd, CARRY_PENDING), value, fs);
}
...
export function recoverCarryPending({ cwd, config, outputPath, outputBinding, pending, discard = false, fs = {} }) {
...
export function assertLifecycleJoins({ review, lane, priorCarryHash, config }) {
  ...
  for (const renewal of review.renewals) {
    ...
    if (!candidate || candidate.state === 'hidden' || candidate.privacy.decision !== 'automatic-safe') {
...
export function deriveNextCarry({ priorCarry, priorCarryHash, review, lane, week, config }) {
  ...
  for (const [lineageRef, entry] of active) {
    ...
    if (entry.automaticThroughWeek === null && entry.manualTargetWeek === null) {
      retired.push(retiredRow({ entry, reason: 'manual-expired', config }));
      active.delete(lineageRef);
    }
  }
  ...
  const retiredFingerprints = new Set(carry.weeks.filter((record) => record.week.start < week.start)
    .flatMap((record) => record.retired)
  ...
  const priorWeeks = carry.weeks.filter((row) => row.week.start < week.start)
    .map((row) => sanitizedPriorRecord(row, config));
