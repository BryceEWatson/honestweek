...
/** Read + parse a JSON registry file. Throws on an unparseable file; callers gate
 *  PRESENCE via existsSync first (an absent file is the opt-in "no goals page" case). */
...
export function validateObjectives({ registry, projectLabels, redactor } = {}) {
  ...
  // The optional page copy + per-group descriptions are ALSO curated strings the
  // goals page renders, so gate them exactly like objective prose (non-empty when
  // present + redactor-stable). Without this the publish gate would skip them and a
  // configured private term could reach goals.html through page.* or groupDescriptions.*.
  const page = registry.page;
  if (page != null) {
    if (typeof page !== 'object' || Array.isArray(page)) {
      errors.push('registry.page must be an object when present');
    } else {
      for (const field of ['eyebrow', 'title', 'lede', 'note']) {
        const text = page[field];
        if (text == null) continue;
        if (typeof text !== 'string' || !text.trim()) {
          errors.push(`registry.page.${field} is present but not a non-empty string`);
          continue;
        }
        checkStable(`page.${field}`, text);
      }
    }
  }
  const gd = registry.groupDescriptions;
  if (gd != null) {
    if (typeof gd !== 'object' || Array.isArray(gd)) {
      errors.push('registry.groupDescriptions must be an object when present');
    } else {
      for (const [g, desc] of Object.entries(gd)) {
        if (!desc || typeof desc !== 'object' || Array.isArray(desc)) {
          errors.push(`groupDescriptions["${g}"] must be an object`);
          continue;
        }
        for (const field of ['teaser', 'what', 'why']) {
          const text = desc[field];
          if (text == null) continue;
          if (typeof text !== 'string' || !text.trim()) {
            errors.push(`groupDescriptions["${g}"].${field} is present but not a non-empty string`);
            continue;
          }
          checkStable(`groupDescriptions["${g}"].${field}`, text);
        }
      }
    }
  }
...
export function validateChangelog({ changelog, registry, redactor } = {}) {
  ...
  for (const c of changes) {
    ...
    for (const f of from) {
      ...
      // The from-label is RENDERED in the change band for EVERY change type (aggregateGoals
      // carries `from` unconditionally), so gate it for every type — not just the four below
      // — else an "add" (or any) change could smuggle an un-redactor-checked label onto the page.
