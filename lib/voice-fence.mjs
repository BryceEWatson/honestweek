...
// allowlist (a superset of validate.mjs's `text ?? summary` — it also reads
// `title`), never a snippet. Each field is scanned AS IT RENDERS — `String(value)`,
// mirroring page.mjs's `esc()` — so an array-valued field (which renders
// comma-joined) cannot smuggle a phrase past the lint, while an object-valued field
// renders as "[object Object]" and is not spuriously scanned (no false abort).
...
// "never scan evidence" guarantee, and these key NAMES are therefore RESERVED:
// authored prose must not be nested under one (editorial lives under mission /
// frontier / blurb / headline / …, never under `commits`/`receipt`/`evidence`), or
// it would be treated as evidence and skipped. (A superset of the commit fields
// validate.mjs's citedShas() reads — primaryCommit, commit(s), candidateCommits —
// plus the snippet/receipt/evidence/proof keys that carry verified quoted text.)
...
// withholding/meta INTENT and catch close paraphrases that reuse the enumerated
// verb/synonym set, while letting an innocuous use of a trigger word pass (a
// "sealed-bid auction", an "honest mistake", a "generic helper", "surfaced a bug").
// They are not exhaustive — a consumer extends the list via voice.denyPhrases and
// carves out a false positive via voice.allowPhrases.
...
export function collectAuthoredProse({ items, content, projects } = {}) {
  ...
  (Array.isArray(items) ? items : []).forEach((item, i) => {
    ...
    // Scan each authored prose field AS IT RENDERS: page.mjs emits esc(String(value)),
    // so we lint String(value). An ARRAY renders comma-joined ("a,b"), so a phrase in
    // (or split across) its elements is caught; an OBJECT renders "[object Object]",
    // whose internals never appear — scanning them would false-abort over prose that
    // is never emitted. Mirroring the renderer with String(value) handles both.
    for (const field of ITEM_PROSE_FIELDS) {
      if (item[field] != null) out.push({ ref, field, path: `${ref}.${field}`, value: String(item[field]) });
    }
  });
