...
// `title`), never a snippet. Each field is WALKED (see collectAuthoredProse), not
// read as a bare string, so an array-/object-valued field that still renders
// (page.mjs `esc()`s `String(value)`) cannot smuggle a phrase past the lint.
...
export function collectAuthoredProse({ items, content, projects } = {}) {
  ...
  (Array.isArray(items) ? items : []).forEach((item, i) => {
    ...
    // Walk each authored prose field rather than reading it as a bare string: an
    // array-/object-valued title/summary/text still renders (page.mjs `esc()`s
    // `String(value)`), so its nested strings must be linted too. walkProse honors
    // EVIDENCE_KEYS, so an evidence subtree nested in a prose field stays unscanned.
    for (const field of ITEM_PROSE_FIELDS) {
      if (item[field] != null) walkProse(item[field], ref, out, `${ref}.${field}`);
    }
  });
