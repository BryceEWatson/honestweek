...
export function curateDigest(promptStore, scannedEvidence, config, week, now = new Date(), {
  ...
  const tombstones = (sameWeek ? priorReview.tombstones ?? [] : []).map((value) => ({ ...value, evidenceRefs: [...value.evidenceRefs] }));
  ...
  const candidates = sourceCandidates.filter((candidate) => !tombstoneByRef.has(candidate.itemRef));
  for (const candidate of candidates) {
  ...
  const review = {
    version: tombstones.length ? 2 : 1, generatedAt: now.toISOString(), week,
