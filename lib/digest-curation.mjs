...
import {
  chooseRetirementReason, retiredRow,
...
function terminalMatches(entry, terminals, config) {
  ...
  const invalid = terminal.isPrivate || terminal.residualRisk !== 'low' || terminal.truncated ||
...
function lifecycleCandidates(sourceCandidates, scannedEvidence, config, week, carry, tombstoneByRef, priorByRef) {
  ...
  for (const entry of record.entries) {
    ...
    const candidate = copyCandidate(current ?? entry.candidate);
    const prior = priorByRef.get(candidate.itemRef);
    if (prior && prior.state !== 'inbox') candidate.state = prior.state;
...
export function curateDigest(promptStore, scannedEvidence, config, week, now = new Date(), {
  ...
  const lifecycle = lifecycleCandidates(
    sourceCandidates, scannedEvidence, config, week, carry, tombstoneByRef, priorByRef,
  );
  const candidates = [
