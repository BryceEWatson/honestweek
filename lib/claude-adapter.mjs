...
/** Find the configured repo whose resolved path contains `cwd`. */
function matchRepo(cwd, config) {
  if (!cwd) return null;
...
function globExt(pattern) {
  ...
  const ext = extOf(base.replace(/\*/g, ''));
  return ext ? `*${ext}` : '*';
}
...
function extractCommits(body, dateISO) {
  const found = [];
  // `git commit` summary: [branch a1b2c3d] Subject
  for (const m of body.matchAll(/\[[^\]\n]*?\b([0-9a-f]{7,40})\]\s*([^\n]*)/g)) {
    found.push({ sha: m[1], date: dateISO, subject: m[2].trim() });
  }
  // `git log --oneline`: a1b2c3d Subject   (line-leading short sha)
  for (const m of body.matchAll(/^([0-9a-f]{7,40})\s+([^\n]+)$/gm)) {
    found.push({ sha: m[1], date: dateISO, subject: m[2].trim() });
  }
  // bare full sha line (rev-parse)
  for (const m of body.matchAll(/^([0-9a-f]{40})\s*$/gm)) {
    found.push({ sha: m[1], date: dateISO, subject: '' });
  }
  return found;
}
...
function shortId(sessionId, file) {
  ...
  const hy = raw.indexOf('-');
  return hy > 0 ? raw.slice(0, hy) : raw.slice(0, 8);
}
