...
export function normalizeAttributionPath(p, cwd = p) {
  const normalized=normalizePath(resolve(p));
  return process.platform==='win32'||/^[A-Za-z]:[\\/]/.test(cwd)?normalized.toLowerCase():normalized;
}

function matchRepoWithRoots(cwd,config,rootsFor){
  if(!cwd)return null;const c=normalizeAttributionPath(cwd,cwd);let best=null,bestLen=-1;
  for(const r of Array.isArray(config?.repos)?config.repos:[]){const configured=r.resolvedPath??r.path??'';if(!configured)continue;for(const root of rootsFor(r,configured)){const rp=normalizeAttributionPath(root,cwd);if(rp&&(c===rp||c.startsWith(`${rp}/`))&&rp.length>bestLen){best=r;bestLen=rp.length;}}}
  return best;
}

/** Pure configured-path attribution. It never expands Git worktrees. */
export function matchConfiguredRepo(cwd,config){return matchRepoWithRoots(cwd,config,(_r,configured)=>[configured]);}

/** Find the configured repo that contains `cwd` (longest-prefix match), or null.
 *  Shared with the site session deriver so the cwd→repo rule has ONE implementation.
 *
 *  A repo is matched against EVERY working tree of its git repository, not just the
 *  configured directory: the configured path, the primary working tree, and every
 *  linked worktree (see lib/worktrees.mjs). So a session started in a worktree that
 *  lives NEXT TO the configured path — the same repository checked out a second time
 *  at a sibling directory — credits that repo instead of falling into "other". Only
 *  ATTRIBUTION widens; commit enumeration still reads the configured path alone, so
 *  a worktree's branch/detached HEAD can never become the published commit basis. */
export function matchRepo(cwd, config) {
  return matchRepoWithRoots(cwd,config,(r,configured)=>r.role==='display'?[configured]:attributionRoots(configured));
}
