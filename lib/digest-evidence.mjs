...
import { createCodexTurnReader } from './codex-records.mjs';
...
// For Codex, `codexTurn` is this row read through the file's createCodexTurnReader.
function humanText(row, source, codexTurn) {
  ...
  return codexTurn?.user ?? null;
}

function assistantText(row, source, codexTurn) {
  if (source === 'claude-code') {
  ...
  return codexTurn?.assistant ?? null;
}
...
export function extractDigestCues(raw, { envelopeKind, observedVerification = false } = {}) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (terminal) {
      ...
      if (envelopeKind === 'human-cue') {
        terminals.push({
          kind: terminal[1].toLowerCase() === 'picked up' ? 'picked-up' : 'ruled-out',
          raw: subject, ordinal: terminalOrdinal,
          invalid: !subject || [...subject].length > 1000,
        });
      }
      continue;
    }
...
function terminalEvidenceFor(rawSubject, terminal, context, config) {
  ...
  return {
    ...
    residualRisk: audit.residualRisk, isPrivate: prompt.isPrivate, invalid: terminal.invalid,
  };
}
...
async function readTurns(file, source, onTurn) {
  ...
  try {
    for await (const line of readBoundedJsonlLines(file)) {
      ...
      const codexTurn = source === 'codex' ? readCodexTurn(row) : null;
      const human = humanText(row, source, codexTurn);
      if (human !== null) {
        finish();
        turn += 1;
      ...
      if (!current) continue;
      const assistant = assistantText(row, source, codexTurn);
      if (assistant !== null) {
