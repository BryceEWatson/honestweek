...
const LABELS = Object.freeze({
  idea: 'ideas', technique: 'techniques', decision: 'decisions', decided: 'decisions',
...
function addCue(out, unsupported, rawText, category, ordinal, envelopeKind, observedVerification) {
  ...
  out.push({ category, raw: text, ordinal });
  return true;
}
...
export function extractDigestCues(raw, { envelopeKind, observedVerification = false } = {}) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    const line = lines[i];
    const labelled = /^\s*(?:[-*+]\s+)?(idea|technique|decision|decided|reversal|reversed|next step)\s*:\s*(.*)\s*$/i.exec(line);
    if (labelled) {
      ordinal += 1;
      const category = LABELS[labelled[1].toLowerCase()];
      if (addCue(cues, unsupported, labelled[2], category, ordinal, envelopeKind, observedVerification)) scannedCount += 1;
      continue;
    }
  ...
  return { cues, unsupported, scannedCount, acceptedCount: cues.length };
}
...
function evidenceFor(rawCue, cue, context, config) {
  ...
  return {
    ...
    discriminator: `${cue.category === 'nextSteps' ? 'next-step' : cue.category.slice(0, -1)}:${cue.ordinal}`,
