 ...
 *  8MB. The cut is not direction-neutral — resolution evidence concentrates at the
 *  END of a session, so a truncated session can only be missed (false negative),
 *  never invented. Truncations are counted in `truncated` so the cost is visible. */
