...
const PSEUDO_PROMPT_RE =
  /^\s*<(system-reminder|command-(message|name|args)|task-notification|local-command|codex_delegation|user-prompt-submit-hook)\b/i;
...
export function resolveCorpora({ corpora, roots = {}, env = process.env } = {}) {
  const wanted = Array.isArray(corpora) && corpora.length ? corpora : ['claude-code', 'codex', 'cowork'];
  const out = [];
  for (const kind of wanted) {
    if (kind === 'claude-code') out.push({ kind, root: roots['claude-code'] ?? resolveProjectsRoot(env) });
      ...
      if (explicit) out.push({ kind, root: explicit });
        ...
        const found = resolveCoworkRoots(env).filter((r) => existsSync(r));
        for (const root of found) out.push({ kind, root });
      }
    }
  }
  return out;
