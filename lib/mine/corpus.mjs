...
//     user   : {type:"response_item", payload:{type:"message", role:"user", content:[...]}}
//              (legacy: {type:"event_msg", payload:{type:"user_message", message}})
//     asst   : {type:"response_item", payload:{type:"message", role:"assistant", ...}}
//              (legacy: {type:"event_msg", payload:{type:"agent_message", message}})
//              Both shapes are read through lib/codex-records.mjs.
...
import { CODEX_HEAD_MAX_BYTES, createCodexTurnReader } from '../codex-records.mjs';

/** Cap on bytes streamed per session. A runaway 18MB transcript must not stall a
 *  scan. ASSUMED, not measured: that the detector's signals appear within the first
 *  8MB, and that the cut mostly costs misses — resolution evidence concentrates at
 *  the END of a session. Not absolute: DISQUALIFYING tail evidence (own-repo edit
 *  dominance, an already-covered error) can be cut too, so a truncated session is
 *  disclosed wherever it produces a draft. Counted in `truncated`. */
...
/** ~/.codex/{sessions,archived_sessions}, or the same under $CODEX_HOME. Archived
 *  sessions are still sessions — a thread the user filed away can hold a solved
 *  problem, and skipping the directory silently shrank the corpus (lib/
 *  prompt-adapters.mjs has always scanned both). */
export function resolveCodexRoots(env = process.env) {
...
export function probeSession(kind, file) {
  if (kind === 'codex') return probeCodexSession(file);
...
/** Codex head sizes tried in turn. The context Codex now writes ahead of the first
 *  human turn pushes about half of sessions past the first step (see
 *  lib/codex-records.mjs for the measured spread). */
const CODEX_HEAD_STEPS = [64 * 1024, 256 * 1024, CODEX_HEAD_MAX_BYTES];

function probeCodexSession(file) {
  for (const maxBytes of CODEX_HEAD_STEPS) {
    const { lines, eof } = readHead(file, maxBytes);
    let startISO = null;
    let cwd = null;
    let isSubagent = false;
    ...
    for (const line of lines) {
      ...
      const { user } = read(rec);
      if (user != null && isHumanText(user)) {
        const firstPromptISO = typeof rec.timestamp === 'string' ? rec.timestamp : startISO;
        return { startISO: firstPromptISO ?? startISO, cwd, firstPrompt: user, firstPromptISO, isSubagent };
      }
    }
    if (eof) return null;
  }
...
export async function streamSession(kind, file, { maxBytes = MAX_SESSION_BYTES } = {}) {
  ...
  const readCodexTurn = createCodexTurnReader();
  let bytes = 0;
  let lines = 0;
  let truncated = false;
  ...
  try {
    for await (const line of rl) {
      ...
      if (kind === 'codex') {
        ...
        const turn = readCodexTurn(rec);
        if (turn.user != null) {
          if (isHumanText(turn.user)) events.push({ kind: 'human', text: turn.user });
        } else if (turn.assistant != null) {
          events.push({ kind: 'assistant', text: turn.assistant });
    ...
    rl.close();
    // rl.close() alone leaves the file open after an early break (the byte cap);
    // across a sweep with many oversized transcripts that leaks fds toward EMFILE.
    input.destroy();
  }
...
export function resolveCorpora({ corpora, roots = {}, env = process.env } = {}) {
  ...
  for (const kind of wanted) {
    ...
    if (kind === 'claude-code') out.push({ kind, root: roots['claude-code'] ?? resolveProjectsRoot(env) });
      ...
      const explicit = roots.codex;
      if (explicit) out.push({ kind, root: explicit });
      else {
        // Same multi-root pattern as cowork below: sessions/ plus archived_sessions/,
        // each judged separately, and one row even when neither exists.
        const candidates = resolveCodexRoots(env);
...
export function enumerateSessions({ corpora, roots, since = null, env = process.env } = {}) {
  ...
  for (const { kind, root } of resolved) {
    ...
    // `probeFailed` counts probed files that yielded NO usable identity. It is how the
    // caller tells "probed and legitimately filtered" (a --since skip, a subagent) from
    // "probed and no longer understood" — the signature of an upstream format change.
    const diag = { kind, root, present: existsSync(root), filesFound: files.length, filesProbed: 0, probeFailed: 0, accepted: 0, oldest: null, newest: null };
