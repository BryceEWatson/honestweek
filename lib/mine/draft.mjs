 ...
 *  see the module header. Recognised by name so a destination can call it whatever
 *  it calls it. */
const VERIFIED_KEYS = new Set(['lastVerified', 'last_verified', 'verifiedOn', 'checked']);
...
export function slugFor(finding) {
  const base = (finding.primaryError || finding.products?.[0]?.name || 'finding')
    .toLowerCase()
    .replace(/<[^>]*>/g, '')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
...
export function renderDraft(finding, { config = {}, now = new Date(), redactor } = {}) {
  ...
  for (const key of Object.keys(shape)) {
    ...
    if (!KNOWN_KEYS.has(key)) {
      lines.push(`${key}: ${yamlScalar(shape[key])}`);
      continue;
    }
    ...
    else if (key === 'draft') lines.push('draft: true');
