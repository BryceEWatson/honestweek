...
export function deidentify(line) {
  // ORDER IS THE WHOLE CORRECTNESS ARGUMENT HERE, and two orderings have already
  // leaked a username:
  //   1. Normalizing separators LAST let `C:\Users\<name>\…` past the home rule and
  //      out as `<drive>/c/Users/<name>`.
  //   2. Collapsing REPEATED separators last let the ordinary JSON-escaped
  //      `C://Users//alice//…` form past it the same way.
  // So: one separator style first, then collapse runs, and only then match. Every
  // rule below sees a single canonical shape and none has to spell `[\\/]`.
  return (
    normSlash(String(line))
      // UNC first, while its leading `//` is still distinguishable from a collapsed
      // separator run. Collapsing first destroys the marker and the share leaks.
      .replace(/(^|\s)\/\/[^/\s]+\/[^/\s]+/g, '$1<share>')
      // THEN collapse repeated separators, in two passes so a URL is not mangled.
      // A drive is ONE letter and a colon (`C://Users//alice` is an ordinary
      // JSON-escaped path); the lookbehind is what stops the same rule matching the
      // final letter of a scheme and turning `https://host` into `https:/host`, which
      // matters because issue URLs are quoted in drafts.
      ...
      //
      // The account segment may contain SPACES — "First Last" is an ordinary Windows
      // account name, and a segment of `[^/\s"']+` stopped at the space and published
      // the surname. It is matched up to the next separator instead, with the trailing
      // separator required so a bare mention of `/Users/` cannot swallow a sentence.
      .replace(/(?:[A-Za-z]:)?(?:\/[A-Za-z])?\/(?:Users|home|Documents and Settings)\/[^/"']+?(?=\/|["']|$)/gi, '<home>')
      ...
      // separators (`C--Users-alex-Projects-SomeProject`). That segment carries the
      ...
      // here (`c:UsersalexProjectsfoo`) matches none of the rules above, yet still
...
/**
 * detectSession(session) -> Promise<finding>
 *
 * Streams one session and ALWAYS returns a finding; `isCandidate` says whether it
 * cleared `classify`, and `rejectedFor` says why not. The finding carries its own raw
 * features so a reader can re-derive the verdict rather than trust it — the same
 * discipline honestweek applies to its git numbers.
 */
