...
/** Gitignore entries `init` ensures in a destination repo, like every sibling
 *  sidecar's. The ledger itself is deliberately TRACKED (it is the only record of
 *  past decisions); what is ignored is its atomic-write temp files and the default
 *  directory for unverified draft posts. */
export const MINE_GITIGNORE = Object.freeze(['honestweek.findings.json.tmp-*', 'honestweek.drafts/']);
