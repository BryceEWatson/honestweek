...
const TEST_COMMAND = /(?:^|[;&|]\s*)(?:node(?:\.exe)?\s+--test\b|npm(?:\.cmd)?\s+(?:run\s+)?test\b|pnpm(?:\.cmd)?\s+(?:run\s+)?test\b|yarn(?:\.cmd)?\s+(?:run\s+)?test\b|python(?:3|\.exe)?\s+-m\s+pytest\b|pytest(?:\.exe)?\b|cargo(?:\.exe)?\s+test\b|go(?:\.exe)?\s+test\b)/i;
const COMMIT_COMMAND = /(?:^|[;&|]\s*)git(?:\.exe)?(?:\s+-C\s+(?:"[^"]+"|'[^']+'|\S+))*\s+commit\b/i;
const COMMIT_OK = /^\[[^\]\r\n]+ [0-9a-f]{7,40}\]/mi;
...
function positiveVerification(kind,result){return kind==='commit'?COMMIT_OK.test(result):VERIFY_OK.test(result);}
...
async function adaptFile(file, source, { config, weekStart, weekEnd }) {
  ...
  const provisional=[];let shellCalls=new Map();
  ...
  for await(const line of lines){
    ...
    if(raw!==null){
      ...
      turn++;shellCalls=new Map();currentPrompt=null;
    ...
    if(source==='claude-code'){
      ...
      if(r?.type==='response_item'&&p?.type==='function_call'&&['shell_command','exec_command'].includes(p.name)&&(p.call_id||p.id)){const kind=verificationKind(p);if(kind)shellCalls.set(p.call_id||p.id,kind);}
      if(r?.type==='response_item'&&p?.type==='function_call_output'&&shellCalls.has(p.call_id||p.id)){const result=typeof p.output==='string'?p.output:JSON.stringify(p.output??'');const kind=shellCalls.get(p.call_id||p.id);const exitCode=explicitExitCode(p.output);if(exitCode!==0||VERIFY_BAD.test(result))currentPrompt._verificationFailed=true;else if(positiveVerification(kind,result))currentPrompt._verificationPassed=true;}
