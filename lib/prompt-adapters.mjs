...
const TEST_COMMAND = /^(?:node(?:\.exe)?\s+--test\b|npm(?:\.cmd)?\s+(?:run\s+)?test\b|pnpm(?:\.cmd)?\s+(?:run\s+)?test\b|yarn(?:\.cmd)?\s+(?:run\s+)?test\b|python(?:3|\.exe)?\s+-m\s+pytest\b|pytest(?:\.exe)?\b|cargo(?:\.exe)?\s+test\b|go(?:\.exe)?\s+test\b)/i;
const COMMIT_COMMAND = /^git(?:\.exe)?(?:\s+-C\s+(?:"[^"]+"|'[^']+'|\S+))*\s+commit\b/i;
const SHELL_CONTROL = /[\r\n;&|]/;
const COMMIT_OK = /^\[[^\]\r\n]+ [0-9a-f]{7,40}\]/mi;
