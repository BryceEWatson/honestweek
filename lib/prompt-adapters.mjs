...
async function adaptFile(file, source, { config, weekStart, weekEnd }) {
  ...
  const provisional=[];const readCodexTurn=createCodexTurnReader();let shellCalls=new Map();let seenCalls=new Set();let seenOutputs=new Set();
  ...
  for await(const line of readBoundedJsonlLines(file)){
    ...
    const raw=source==='claude-code'?claudeText(r):readCodexTurn(r).user;
    if(raw!==null){
