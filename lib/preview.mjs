 ...
 */
export function startServer({ host = LOOPBACK, port = 0, html = '' } = {}) {
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      res.writeHead(200, {
        ...
        // default-src 'none' blocks every external fetch; inline CSS is the only
        // thing allowed. This turns "no egress from the page" into a guarantee
        // the browser enforces, not just a code-review promise.
...
const HELP = `honestweek preview: render the built output as HTML on a local-only server.

Usage:
  honestweek preview [--file <path>] [--port <n>] [--no-open]

Reads the Markdown that build wrote (config.output.file, or --file <path>),
converts it to a self-contained HTML page, and serves it on 127.0.0.1 (loopback
only). Nothing is published and nothing leaves your machine.
...
export async function runPreview({
  ...
  const html = renderPage(mdToHtml(markdown), { title: titleFromMarkdown(markdown) });

  let handle;
  try {
    handle = await startServer({ host: LOOPBACK, port, html });
  } catch (err) {
