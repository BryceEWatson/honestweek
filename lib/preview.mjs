...
/**
 * startServer({ host, port, html, routes }) -> Promise<{ server, url, port, host, close }>
 * Binds loopback ONLY (host defaults to 127.0.0.1) and serves under a strict CSP
 * that ENFORCES the "no external resources" promise. By default it serves `html`
 * at `/`; pass `routes` (a `{ '<path>': html }` map) to serve MULTIPLE pages (the
 * standalone report + goals pair), each under the same CSP. Every other path 404s.
 * Resolves once listening (port 0 yields an OS-assigned free port).
 */
export function startServer({ host = LOOPBACK, port = 0, html = '', csp = "default-src 'none'; style-src 'unsafe-inline'", routes = null } = {}) {
  // The path -> body table. A single-file preview is just the one '/' route, so the
  // multi-page and single-page paths share exactly one serving codepath.
  const table = routes && typeof routes === 'object' ? routes : { '/': html };
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      res.end(method === 'HEAD' ? undefined : table[path]);
    });
...
wrote complete HTML, so it is served verbatim (with its inline interactivity). If
a goals.html sits beside the previewed report (the opt-in standalone goals build),
both pages are served so their cross-links resolve. Either way it is served on
127.0.0.1 (loopback only) under a no-external-egress CSP. Nothing is published and
...
export async function runPreview({
  ...
  // Multi-page: when the previewed HTML report has a goals.html sibling (the opt-in
  // standalone goals build), serve BOTH so the cross-links (report.html <-> goals.html)
  // resolve. '/' and the report's own filename both serve the report. Still loopback-
  // only, still under the script-permitting no-egress CSP. Single-file path unchanged.
  let routes = null;
  if (isHtml) {
    const base = basename(mdPath);
    const goalsPath = join(dirname(mdPath), 'goals.html');
    if (base.toLowerCase() !== 'goals.html' && existsSync(goalsPath)) {
      try {
        const goalsHtml = readFileSync(goalsPath, 'utf8');
        routes = { '/': html, [`/${base}`]: html, '/goals.html': goalsHtml };
      } catch {
        /* goals.html unreadable — fall back to single-page (report only) */
      }
    }
  }
  ...
  let handle;
  try {
    handle = await startServer({ host: LOOPBACK, port, html, csp, routes });
  } catch (err) {
  ...
  io.out(
    `honestweek preview: serving ${mdPath} at ${handle.url}` +
      (routes ? ` (+ ${handle.url}goals.html)` : '') +
