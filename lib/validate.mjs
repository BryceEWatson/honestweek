 ...
 */
export async function runValidate({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), promptLockHeld = false } = {}) {
  if (!promptLockHeld) {
    let code;
    try {
      ...
      code = await withPromptLock(
        cwd,
        () => runValidate({ cwd, argv, now, io: { out: io.out, err: io.err, exit: (value) => value }, promptLockHeld: true }),
        { ensureIgnored: false },
      );
    } catch (err) {
