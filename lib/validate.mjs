...
import { assertNoDigestGroupCollision, DIGEST_KINDS, isReservedDigestItem } from './digest-schema.mjs';
import { readCarryPending, recoverCarryPending } from './digest-carry.mjs';
...
export function validateItems(items, config, { noDashes = false } = {}) {
  ...
  (Array.isArray(items) ? items : []).forEach((item, index) => {
    ...
    const sessionMaterial = item.kind === 'prompt' ||
      (DIGEST_KINDS.includes(item.kind) && item.publicDisposition === 'automatic-safe');
    if (sessionMaterial ? item.status !== '' : item.status != null && !STATUSES.includes(item.status)) {
 ...
 */
export async function runValidate({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), promptLockHeld = false, transactionFs = {} } = {}) {
  if (!promptLockHeld) {
    let code;
    try {
      ...
      code = await withPromptLock(
        cwd,
        () => runValidate({ cwd, argv, now, io: { out: io.out, err: io.err, exit: (value) => value }, promptLockHeld: true, transactionFs }),
        { ensureIgnored: false },
      );
    } catch (err) {
      io.err(`validate: ABORTED — ${err.message}\nNo output was written.\n`);
      code = 2;
    }
  ...
  try {
    ...
    io.err(`validate: ABORTED â€” ${err.message}\n`);
    return io.exit(2) ?? 2;
  }
  ...
  if (items.some(isReservedDigestItem)) {
    io.err(`validate: ABORTED: ${ITEMS_FILE} cannot contain reserved digest-lane item fields.\nNo output was written.\n`);
    return io.exit(2) ?? 2;
  }
  const parsedWeek = Array.isArray(parsed) ? null : parsed.week;
  let week;
  if (parsedWeek && (parsedWeek.start || parsedWeek.weekStart)) {
    week = { start: String(parsedWeek.start ?? parsedWeek.weekStart).slice(0,10), end: String(parsedWeek.end ?? parsedWeek.weekEnd).slice(0,10) };
  } else {
    const r = resolveWeek({ today: localDateInTimezone(now, config.week.timezone) });
    week = { start: r.weekStart.toISOString().slice(0,10), end: r.weekEnd.toISOString().slice(0,10) };
  }
  try {
    const promptLane = await loadValidatedPromptLane({ cwd, config, week, now, hasGoals: existsSync(join(cwd, 'honestweek.objectives.json')) });
    assertNoDigestGroupCollision(items, promptLane.lane, Array.isArray(parsed) ? [] : parsed.projects, config.repos);
    items = [...items, ...promptLane.items];
  } catch (err) {
    io.err(`validate: ABORTED â€” ${err.message}\nNo output was written.\n`);
    return io.exit(2) ?? 2;
  }
