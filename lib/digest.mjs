...
import {
  CARRY_GITIGNORE, readCarry, readCarryPending, recoverCarryPending, writeCarry,
...
function readPriorControlReview(cwd) {
  let value;
  try { value = JSON.parse(readFileSync(join(cwd, 'honestweek.curated.json'), 'utf8')); }
  catch (error) { throw new Error(`honestweek.curated.json is not valid JSON (${error.message}).`); }
  if (![1,2,3].includes(value?.version) || !value.week || !/^\d{4}-\d{2}-\d{2}$/.test(value.week.start) ||
      !/^\d{4}-\d{2}-\d{2}$/.test(value.week.end) || !Array.isArray(value.candidates) ||
      (value.version >= 2 && !Array.isArray(value.tombstones)) ||
      (value.version === 3 && !Array.isArray(value.renewals))) throw new Error('digest review control state is invalid.');
  ...
  for (const renewal of value.renewals ?? []) {
    if (!renewal || JSON.stringify(Object.keys(renewal).sort()) !== JSON.stringify(['itemRef','requestedAt','targetWeek']) ||
        !/^[0-9a-f]{64}$/.test(renewal.itemRef) || Number.isNaN(new Date(renewal.requestedAt).getTime()) ||
        !/^\d{4}-\d{2}-\d{2}$/.test(renewal.targetWeek)) throw new Error('digest review renewal state is invalid.');
  }
  return value;
}
...
function renewedReview(review, candidate, now) {
  if (candidate.state === 'hidden' || candidate.decision !== 'automatic-safe') {
    throw new Error('carry-forward requires a live selected public-safe candidate.');
  }
  const next = structuredClone(review);
...
async function resetTombstones({ cwd, config, args, now, io, transactionFs = {} }) {
  ...
  const digestRows = [
    ...(carryLoaded.value?.tombstones ?? []).map((row) => ({ source: 'carry', row })),
    ...(review?.tombstones ?? []).map((row) => ({ source: 'review', row, week: review.week })),
  ];
  const promptRows = promptStore?.tombstones ?? [];
  const digestItemRefs = new Set();
  const promptRefs = new Set();
  if (all) {
    for (const value of digestRows) digestItemRefs.add(value.row.itemRef);
    ...
    for (const value of digestRows) {
      const sourceWeek = value.row.week ?? value.week;
      if (sourceWeek?.start === target.start && sourceWeek?.end === target.end) {
        digestItemRefs.add(value.row.itemRef);
        for (const ref of value.row.evidenceRefs) promptRefs.add(ref);
      }
    }
    for (const value of promptRows) {
      if (value.week?.start === target.start && value.week?.end === target.end) promptRefs.add(value.ref);
    }
  } else {
    ...
    for (const value of digestRows) {
      if (value.row.itemRef === identity || value.row.evidenceRefs.includes(identity)) {
        digestItemRefs.add(value.row.itemRef);
        for (const ref of value.row.evidenceRefs) promptRefs.add(ref);
      }
    }
  ...
  for (const value of digestRows) {
    if (digestItemRefs.has(value.row.itemRef)) {
      for (const ref of value.row.evidenceRefs) promptRefs.add(ref);
    }
  }
  const carryRemoved = carryLoaded.value?.tombstones.filter((row) => digestItemRefs.has(row.itemRef)).length ?? 0;
  const reviewRemoved = (review?.tombstones ?? []).filter((row) => digestItemRefs.has(row.itemRef)).length;
  ...
  if (carryRemoved) {
    ...
    next.tombstones = next.tombstones.filter((row) => !digestItemRefs.has(row.itemRef));
...
async function recover({ cwd, config, args, io, transactionFs = {}, quiet = false }) {
  if (!readCarryPending(cwd, config, { optional: true })) {
    if (!quiet) io.out('digest recover: no carry transaction is pending.\n');
    return 0;
  }
  ...
  const result = recoverCarryPending({
  ...
  if (!quiet) io.out(`digest recover: ${result.action} the hash-bound carry transaction.\n`);
  return 0;
}
...
export async function runDigest({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), roots, transactionFs } = {}) {
  ...
  try {
    ...
    return await withPromptLock(cwd, async () => {
      if (command === 'recover') return recover({ cwd, config, args, io, transactionFs });
