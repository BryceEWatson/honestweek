...
async function prepare({ cwd, config, args, now, roots, io, transactionFs = {} }) {
  ...
  const { review, lane } = curateDigest(promptStore, evidence, config, { start: week.start, end: week.end }, now, { outputBinding });
  ...
  io.out(`digest prepare: selected ${lane.items.length}/${lane.policy.maxItems}; automatic floor ${lane.policy.automaticMinScore}; caps ${JSON.stringify(lane.policy.categoryCaps)}. Eligible omissions: ${omitted}. Privacy withheld: ${privacy}. Claude Code ${lane.sourceStatus['claude-code'].state}; Codex ${lane.sourceStatus.codex.state}. Selection favors explicit decisions, reversals, recurrence, and observed verification, not universal importance. Project handoffs are not mined in Slice 2.\n`);
  return 0;
}
...
export async function runDigest({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), roots, transactionFs } = {}) {
  ...
  if (!command || ['-h','--help'].includes(command)) {
    io.out('Usage: honestweek digest <prepare|candidates|explain> [options]\n');
    return 0;
  }
  let config;
