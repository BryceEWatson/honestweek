 ...
 * genuinely active. Session-active days are attributed by the project's CONFIG
 * REPO-LABEL only (the key sessions bucket under) — the catch-all `'other'` is never
 * credited to a named project. Both inputs are deterministic counts (git / on-disk
 * session logs), never authored, so the fact-fence can still trace every number.
 * `sessions` is optional: absent, `daysActive` is the commit-only count (unchanged).
 */
export function deriveProjectStats(richItems, chart, weekStartKey, weekEndKey, sessions) {
  ...
  // Session-active days are attributed by a project's CONFIG REPO-LABEL (_repo) — the key the
  // session bundle buckets under (sessions.days[].byProject). A bucket with no repo gets no
  // session-day credit (an honest under-count). We never read the free-form bucket key or the
  // catch-all 'other': either could mis-attribute another project's (or unconfigured) sessions.
  ...
  for (const project of Object.keys(stats)) {
    ...
    stats[project].daysActive = Math.max(commitDays, sessionDays(repo));
