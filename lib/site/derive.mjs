...
/**
 * deriveProjectStats(items, chart, verifiedIndex) -> { [project]: { entries, statusCounts, daysActive } }
 *
 * Per-project aggregate COUNTS over the items + chart. These are the numbers a
 * grouped site card shows that are not already in chart/sessions (entries,
 * status tallies, active-day count). honestweek computes them (generic: grouped by
 * the item's own `project`) and seeds them into verifiedNumbers, so a transform
 * uses them rather than inventing a count the fence couldn't trace.
 */
export function deriveProjectStats(items, chart, verifiedIndex) {
  const repoTotalsDays = chart?.days ?? [];
  const stats = {};
  ...
  for (const project of Object.keys(stats)) {
    ...
    stats[project].daysActive = repo ? repoTotalsDays.filter((d) => (d.byRepo[repo] || 0) > 0).length : 0;
...
export function augmentSiteModel(model, { config, items, verified, verifiedIndex, week, now, projectsRoot, content, projects } = {}) {
  ...
  const projectStats = deriveProjectStats(items, chart, verifiedIndex);
  ...
  const richItems = (Array.isArray(items) ? items : []).map((item) => ({
    ...
    date: itemDate(item, verifiedIndex),
    verified: true,
  }));
