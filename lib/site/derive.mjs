...
/**
 * deriveProjectStats(richItems, chart, weekStartKey, weekEndKey) ->
 *   { [project]: { entries, statusCounts, daysActive } }
 *
 * Per-project aggregate COUNTS over the IN-WEEK items + chart. These are the
 * numbers a grouped site card shows that are not already in chart/sessions
 ...
 * honestweek computes them (generic: grouped by the item's own `project`) and seeds
 * them into verifiedNumbers, so a transform uses them rather than inventing a count
 * the fence couldn't trace.
 */
export function deriveProjectStats(richItems, chart, weekStartKey, weekEndKey) {
  ...
  for (const project of Object.keys(stats)) {
    ...
    stats[project].daysActive = repo ? days.filter((d) => (d.byRepo[repo] || 0) > 0).length : 0;
...
export function augmentSiteModel(model, { config, items, verified, verifiedIndex, week, now, projectsRoot, content, projects } = {}) {
  ...
  const projectStats = deriveProjectStats(richItems, chart, weekStartKey, weekEndKey);
