...
/**
 * groupInWeekByProject(richItems, weekStartKey, weekEndKey) ->
 *   Map<name, { repo, dates: Set<string>, entries, statusCounts }>
 *
 * Group the IN-WEEK items by their project key (`item.project ?? item.repo`), recording each group's
 * first-seen repo label, its Set of distinct in-week entry dates, its entry count, and its status tally.
 * ONE place owns the in-week window filter + the project-key rule, so `deriveProjectStats` (which derives
 * `daysActive`) and `reconcileGeneralizedSessionTotals` (which derives the per-project session floor) can
 * never drift on HOW a group is formed or which dates count — the drift that would silently desync the
 * active-day count from the session count this whole reconciliation exists to keep in step.
 */
/**
 * The ONE in-week window test, by derived date key. Everything that asks "does this
 * belong to the week being reported?" goes through here so the answer cannot differ
 * between the feed, the per-project stats and the provenance counts.
 */
function isInWeekDate(dateKey, weekStartKey, weekEndKey) {
  return typeof dateKey === 'string' && dateKey !== '' && dateKey >= weekStartKey && dateKey <= weekEndKey;
}

/** The same test for an item, using its derived date. */
function isInWeekItem(item, weekStartKey, weekEndKey) {
  return isInWeekDate(item?.date, weekStartKey, weekEndKey);
}

function groupInWeekByProject(richItems, weekStartKey, weekEndKey) {
  const per = new Map();
  for (const item of Array.isArray(richItems) ? richItems : []) {
    if (!isInWeekItem(item, weekStartKey, weekEndKey)) continue;
    const project = item.project ?? item.repo ?? null;
    if (project == null) continue;
    let info = per.get(project);
    if (!info) {
      info = { repo: item.repo ?? null, dates: new Set(), entries: 0, statusCounts: {} };
      per.set(project, info);
    }
    info.entries += 1;
    const status = item.status ?? statusFor(item);
    info.statusCounts[status] = (info.statusCounts[status] || 0) + 1;
    info.dates.add(item.date);
  }
  return per;
}
...
export function deriveProjectStats(richItems, chart, weekStartKey, weekEndKey, sessions) {
  ...
  const stats = {};
  for (const [project, info] of groupInWeekByProject(richItems, weekStartKey, weekEndKey)) {
    const repo = info.repo;
    const commitDays = repo ? days.filter((d) => (d.byRepo[repo] || 0) > 0).length : 0;
    // Session-day key: a git-backed project uses its config repo-label (repo); a repo-less display /
    // session-only project (repo null) falls back to its STATS KEY (the project name) — the same key
    ...
    // Entries-derived floor (cross-cwd reconciliation, see reconcileGeneralizedSessionTotals' doc): a
    // group already shown as a commit/session source (activeSignal > 0) is lifted to at least its distinct
    // in-week entry-day count, so its "active D days" can never undercount the rows a consumer renders
    // beneath it. The activeSignal > 0 gate is not a vibe boundary: it is the SAME line that decides
    // whether this project shows an activity tally AT ALL. A project with >= 1 commit- or session-day has
    // INDEPENDENT evidence it was a live project this week, so crediting its curated entry-days as
    // active-days reconciles two views (cwd bundle vs. content) of a known-active project. A project with
    // ZERO commit- and session-days has NO such independent evidence — its only signal is the curated
    // rows themselves — so honestweek conservatively shows those rows WITHOUT manufacturing an active-day
    // count (daysActive stays 0, the consumer omits the "active D days" clause). That is the same choice
    // that keeps the catch-all 'other' pool and the #47 repo-less-no-session thread at 0.
    const entryDays = info.dates.size;
    const daysActive = activeSignal > 0 ? Math.max(activeSignal, entryDays) : activeSignal;
    stats[project] = { entries: info.entries, statusCounts: info.statusCounts, daysActive };
  }
  return stats;
}
 ...
 * in-week entry-day count, so the teaser can never undercount the rows beneath it.
 *
 * WHAT THE LIFTED VALUE MEANS (construct honesty — a session count is not a day count):
 *   The raw `projectTotals[key]` is a tally of session-LOG FILES whose cwd matched this project. The
 *   lifted value is NOT that; it is a LOWER BOUND on the project's distinct session-active DAYS, derived
 *   from its curated entry-days. The inference is: (a) a generalized group has NO config repo, so it has
 *   NO commits — every one of its curated in-week entries is session-derived work, so each entry-day is a
 *   day the project had a session (possibly one bucketed under another project's cwd); and (b) a session
 *   runs on exactly one calendar day, so N distinct session-active days imply >= N sessions. Hence
 *   entry-DAYS (never entry COUNT) is the floor: 2 rows on 2 days imply >= 2 sessions; 3 rows on 2 days
 *   still imply only >= 2, never 3. This is the same "a project can't be active on more days than it had
 *   sessions" invariant that keeps `sessionsThisWeek >= daysActive`; the lifted teaser reports that
 *   floor, a distinct-session-DAY count, not a raw session-file tally. State it as such in any UI copy.
 *
 * PARTITION NOTE (an invariant this DELIBERATELY relaxes): before reconciliation `sessions.projectTotals`
 * is a strict PARTITION of `sessions.interactiveTotal` (deriveSessions increments exactly one bucket +
 * the total per session). Lifting a generalized bucket does NOT touch `total`/`interactiveTotal` (a
 * cross-cwd session is one real session; inflating the deduplicated total would be the dishonest move), so
 * AFTER reconciliation `sum(projectTotals)` can EXCEED `total`. That is intentional: a cross-cwd session
 * legitimately counts toward two projects' day-spans (its cwd project AND the project its work was curated
 * to). A consumer must therefore NOT render `sum(projectTotals)` as if it equalled `total`; the two are no
 * longer a partition. The bumped value is a deterministic distinct-day count in the trusted `sessions`
 * root, so it is fact-fence-ELIGIBLE (seedVerifiedNumbers walks it) — but note the fence checks provenance
 * (no fabricated number), not this partition relationship, which is why it is documented here and pinned
 * by a test rather than caught by the fence.
 *
...
export function reconcileGeneralizedSessionTotals(sessions, richItems, config, weekStartKey, weekEndKey) {
  ...
  // Group by the SAME shared helper deriveProjectStats uses, so the in-week window + project-key rule
  // (and thus which dates count) cannot drift between the active-day floor and this session floor.
  for (const [name, info] of groupInWeekByProject(richItems, weekStartKey, weekEndKey)) {
...
/**
 * deriveProvenance({ items, verified, weekStartKey, weekEndKey })
 *   -> { itemsTotal, itemsVerified, commitsVerified, redactions }
 *
 * Counts for THE WEEK BEING REPORTED. This block lives inside a per-week artifact,
 * next to `weekOf`/`weekStart`/`weekEnd`, so the three COUNTS OF CONTENT --
 * `itemsTotal`, `itemsVerified`, `commitsVerified` -- must describe that week and
 * nothing else.
 *
 * `redactions` is the one exception and is deliberately NOT week-scoped (see its note
 * below). It counts an operation on the write rather than content in the week, so do
 * not read this block as uniformly week-scoped.
 *
 * It used to count the whole authored corpus instead. `items` is the full authored
 * set (all weeks), and only the feed was ever week-filtered, so a report whose page
 * showed 23 entries carried `itemsTotal: 304` -- the total number of items ever
 * authored -- and `commitsVerified: 114` beside a week with 43 commits. Measured
 * across the published /log archive, `itemsTotal` rose monotonically week over week
 * (70, 70, 90, 123, 211, 226, 281, 304) while the entries beneath it did not. A
 * provenance block that overstates by 13x is worse than none, because its name
 * promises it was counted.
 *
 * Pass `richItems` (dates already derived), not the raw authored items: an item's
 * week placement comes from its verified commit date, which raw items do not carry.
 *
 * `redactions` is left 0 here and filled by the build AFTER the redaction pass (its
 * true value is the redactor's hit count for that write). It is the ONE field in this
 * block that is not week-scoped, on purpose: the redactor runs once over the whole
 * emitted bundle, and that bundle carries the full `richItems` corpus, so a scrub
 * triggered by an out-of-week item still lands in this count. That is the honest
 * reading of the field. It says how much this WRITE scrubbed, not how much the week
 * contained, and week-scoping the number would mean reporting a redaction pass that
 * never ran. Cite it as a property of the write, never as a fact about the week.
 *
 * `itemsVerified` deliberately equals `itemsTotal`: the build aborts before this runs
 * unless every cited commit resolved, so every published item is a verified one. It is
 * a restatement rather than an independent measurement, and it is kept only because
 * consumers already read the field.
 *
 * `commitsVerified` counts re-derived commits whose own author-date falls in the week
 * -- the same date basis the chart buckets by -- so it cannot count a commit the week
 * did not contain.
 */
export function deriveProvenance({ items, verified, weekStartKey, weekEndKey }) {
  const itemsTotal = (Array.isArray(items) ? items : []).filter((i) =>
    isInWeekItem(i, weekStartKey, weekEndKey),
  ).length;
  const commitsVerified = (Array.isArray(verified) ? verified : []).filter((v) =>
    isInWeekDate(typeof v?.dateISO === 'string' ? v.dateISO.slice(0, 10) : null, weekStartKey, weekEndKey),
  ).length;
  return {
    itemsTotal,
...
export function augmentSiteModel(model, { config, items, verified, verifiedIndex, week, now, projectsRoot, content, projects } = {}) {
  ...
  // Counted AFTER richItems, because an item's week placement comes from its verified
  // commit date and the raw authored items do not carry one yet. Counting the raw set
  // here is what made every report's provenance describe the whole corpus.
  const provenance = deriveProvenance({ items: richItems, verified, weekStartKey, weekEndKey });
