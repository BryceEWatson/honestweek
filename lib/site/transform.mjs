...
// prose-scanned here. In transform mode the site's strings are TRUSTED CURATED
// content (the target owns and reviews them, redacted), not honestweek's to
// number-check — so a quantity that appears in curated prose ("8 of 13 claims
// drifted") passes, exactly as it did before honestweek. The guarantee is:
// no unverified NUMBER (numeric leaf) reaches the page; curated prose is trusted.
//
