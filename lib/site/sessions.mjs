 ...
 *  adapter uses: a session counts for the repo whose configured root CONTAINS its
 *  cwd. So a session started in a git worktree (<repo>/.claude/worktrees/<name>) or
 *  a subdirectory (<repo>/<sub>) credits <repo>, not the catch-all 'other'. This is
 ...
 *  error.
 *
 *  A SEPARATE sibling repo that merely shares a name prefix (<repo>-parity) is NOT
 *  under <repo>/, so matchRepo's trailing-'/' boundary correctly leaves it 'other'
 *  rather than over-crediting <repo>. */
