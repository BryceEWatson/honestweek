 ...
 *  hero with the digest — the two can no longer disagree on which project a session
 *  belongs to, so the per-project session count beside a project's items can't read
 *  lower than the sessions that produced them. A SEPARATE sibling repo that merely
 *  shares a name prefix (<repo>-parity) is NOT under <repo>/, so matchRepo's
 *  trailing-'/' boundary correctly leaves it 'other' rather than over-crediting <repo>. */
