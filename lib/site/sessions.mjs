...
// is surfaced, not hidden, in the emitted filesScanned/automatedExcluded/
// undetermined/duplicatesSkipped diagnostics — an adapter should not render the
// headline total as an exact, verified session count.
//
// Zero runtime dependencies: Node built-ins only.

import { statSync } from 'node:fs';

import { enumerateSessionFiles, resolveProjectsRoot, matchRepo, readHead } from '../claude-adapter.mjs';
 ...
 *  TARGET's config, never hardcoded here). Reuses the adapter's matchRepo so the
 *  cwd→repo rule has ONE implementation. Returns 'other' for an unconfigured cwd. */
function labelResolver(config) {
  return (cwd) => {
    const repo = matchRepo(cwd, config);
    return repo ? repo.label : 'other';
  };
}
