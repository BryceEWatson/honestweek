...
import { enumerateSessionFiles, resolveProjectsRoot, readHead, matchRepo } from '../claude-adapter.mjs';
 ...
 *
 *  Attribution is PREFIX-based (longest-match), via the SAME matchRepo the digest
 *  adapter uses: a session counts for the repo one of whose WORKING TREES contains
 *  its cwd. So a session started in a git worktree (<repo>/.claude/worktrees/<name>)
 *  or a subdirectory (<repo>/<sub>) credits <repo>, not the catch-all 'other'. This is
 *  how real multi-session work is launched (a worktree per task), and it unifies the
 *  ATTRIBUTION RULE with the digest — the two can no longer disagree on WHICH project
 *  a session belongs to, so a project's count no longer drops the worktree sessions
 *  that produced its items. It does NOT make the hero and the digest count the same
 *  SET of sessions: the hero still applies filters the digest does not (ephemeral-cwd
 *  exclusion, the interactive classifier, first-prompt-timestamp dedup), so it can
 *  still read lower for those reasons — see this module's header on that two-sided
 *  error.
 *
 *  "Working tree" means every checkout of the repo's git repository, not only the
 *  configured directory — so a worktree that lives NEXT TO it (<repo>-<task>, or
 *  anywhere else on disk) credits <repo> too, which is what lets a build launched
 *  from a dedicated sibling worktree still count the sessions run in the primary
 *  checkout. A SEPARATE CLONE that merely shares a name prefix (<repo>-parity) has
 *  its own git database, never appears in <repo>'s worktree list, and correctly
 *  stays 'other' rather than over-crediting <repo>. */
function labelResolver(config) {
  return (cwd) => matchRepo(cwd, config)?.label ?? 'other';
}
