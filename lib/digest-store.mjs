...
const HEX = /^[0-9a-f]{64}$/;
const REVIEW_KEYS = ['version','generatedAt','week','sourceStatus','policy','candidates','withheld'];
...
function validateCandidate(candidate, config, policy) {
  ...
  const allowedReasonCodes = new Set([...DIGEST_SIGNALS, ...DIGEST_DECISIONS, 'explicit-keep']);
...
export function validateDigestReview(review, config) {
  exact(review, REVIEW_KEYS, 'digest review');
  if (review.version !== 1 || !validIso(review.generatedAt) || !Array.isArray(review.candidates)) throw new Error('digest review schema is invalid.');
  ...
  for (const candidate of review.candidates) {
    validateCandidate(candidate, config, review.policy);
...
function validateItem(item, config, policy) {
  ...
  if (!HEX.test(item.itemRef) || item.id !== `digest-${item.itemRef}` || !DIGEST_CATEGORIES.includes(item.category) ||
      ...
      !['kept','automatic'].includes(item.curationState) || item.publicDisposition !== 'automatic-safe' ||
  ...
  if (item.curationState === 'kept' && item.category !== 'prompts') throw new Error('only prompt items can carry Slice 2 keep state.');
  ...
  const validReasonCodes = new Set([...DIGEST_SIGNALS, 'explicit-keep']);
  ...
  const signalCodes = item.selection.reasonCodes.filter((value) => value !== 'explicit-keep');
  ...
  if (item.curationState === 'kept' ? item.selection.reasonCodes[0] !== 'explicit-keep' : item.selection.reasonCodes.includes('explicit-keep')) throw new Error('digest selection keep state is inconsistent.');
...
export function validateDigestLane(lane, config) {
  ...
  const kept = lane.items.filter((item) => item.curationState === 'kept').length;
  const automatic = lane.items.filter((item) => item.curationState === 'automatic');
  if (automatic.length > Math.max(0, lane.policy.maxItems - kept)) throw new Error('digest lane exceeds the overall automatic target.');
  for (const category of DIGEST_CATEGORIES) {
