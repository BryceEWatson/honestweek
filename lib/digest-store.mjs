...
import {
  addWeeks, exactObject as exact, validateLifecycleEntry, validateLifecycleWeek,
  validateRenewal, validateRetired, validIso,
...
function validateWeek(week, name = 'digest week') {
  validateLifecycleWeek(week, name);
}
...
function validatePolicy(policy, { historical = false } = {}) {
  ...
  if (policy.maxItems < 1 || policy.maxItems > 50 || policy.automaticMinScore < -10 || policy.automaticMinScore > 10 ||
      policy.maxAutomaticChangedPercent < 0 ||
      policy.maxAutomaticChangedPercent > (historical ? 100 : 20)) throw new Error('digest policy is out of bounds.');
...
export function validateDigestCandidate(candidate, config, policy = digestPolicyForConfig(config), { historical = false } = {}) {
  ...
  const promptReceipts = candidate.receipts.filter((value) => value.kind === 'human-prompt');
  if (candidate.category === 'prompts'
    ? candidate.receipts.length !== 1 || candidate.receipts[0].kind !== 'human-prompt'
    : cueReceipts.length !== 1 || (candidate.category === 'techniques' || cueReceipts[0].kind === 'human-cue'
      ? candidate.receipts.length !== 2 || promptReceipts.length !== 1
      : candidate.receipts.length !== 1 || promptReceipts.length !== 0)) throw new Error('digest candidate receipt shape is invalid.');
  ...
  if (!historical && candidate.score !== expectedScore) throw new Error('digest candidate score does not match policy weights.');
...
export function validateDigestReview(review, config, { historical = false } = {}) {
  ...
  validatePolicy(review.policy, { historical });
  if (!historical) validatePolicyAgainstConfig(review.policy, config);
  validateWithheld(review.withheld);
  const seen = new Set();
  for (const candidate of review.candidates) {
    validateDigestCandidate(candidate, config, review.policy, { historical });
  ...
  if (review.version === 3) {
    ...
    for (const renewal of review.renewals) {
      ...
      if (renewal.targetWeek !== addWeeks(review.week.start, 1)) {
        throw new Error('digest renewal target is not the next reporting week.');
      }
    ...
    for (const retired of review.lifecycle.retired) {
      validateRetired(retired, config, { historical });
      const key = `${retired.lineageRef}\0${retired.itemRef}`;
      if (retiredRefs.has(key)) throw new Error('digest lifecycle has duplicate retired lineage/item.');
      retiredRefs.add(key);
    }
...
export function readDigestReview(cwd, config, { historical = false } = {}) {
  ...
  return validateDigestReview(value, config, { historical });
}
