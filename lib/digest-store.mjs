...
export function validateDigestReview(review, config) {
  exact(review, review?.version === 2 ? REVIEW_V2_KEYS : REVIEW_V1_KEYS, 'digest review');
  if (![1,2].includes(review.version) || !validIso(review.generatedAt) || !Array.isArray(review.candidates) ||
      (review.version === 2 && !Array.isArray(review.tombstones))) throw new Error('digest review schema is invalid.');
