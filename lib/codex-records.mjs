...
/** The IDE extension wraps a person's request in its own context (open file, open
 *  tabs, mentioned files) and marks where the request starts. Only the text after
 *  that marker is the person's; a block with no marker is all context. */
const IDE_CONTEXT_RE = /^# (?:Context from my IDE setup|Files mentioned by the user):/;
...
function personText(blocks) {
  ...
  for (const raw of blocks) {
    ...
    if (IDE_CONTEXT_RE.test(t)) {
      const at = t.indexOf(IDE_REQUEST_MARKER);
      const request = at < 0 ? '' : t.slice(at + IDE_REQUEST_MARKER.length).trim();
      if (request) kept.push(request);
      continue;
    }
