...
export function mergePromptStore(oldStore, scanned, now = new Date()) {
  ...
  const staged = scanned.prompts.filter((p) => !deleted.has(p.ref)).map((p) => {
    ...
    const { _residualRisk, _followOnCorrectionTurn, ...persisted } = p;
    return { ...persisted, state: old?.state ?? 'inbox', _followOnCorrectionTurn };
  });
  const eligibleTurns = new Set(staged.filter((p) => p.state !== 'hidden' && !p.isPrivate)
    .map((p) => `${p.source}\0${p.sessionKey}\0${p.turn}`));
  const prompts = staged.map(({ _followOnCorrectionTurn, ...p }) => ({
    ...p,
    followOnCorrection: Number.isInteger(_followOnCorrectionTurn) &&
      eligibleTurns.has(`${p.source}\0${p.sessionKey}\0${_followOnCorrectionTurn}`),
  }));
