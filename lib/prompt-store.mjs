...
const TOMBSTONE_KEYS = ['ref','refCanonical','source','sessionKey','turn','deletedAt'];
...
export function validatePromptStore(store) {
  ...
  if (store.version !== 1 || !Array.isArray(store.prompts) || !Array.isArray(store.tombstones)) throw new Error('prompt store has an unsupported schema.');
  ...
  for (const t of store.tombstones) { exact(t,TOMBSTONE_KEYS,'tombstone');validatePromptIdentity(t, 'tombstone'); if (seen.has(t.ref)) throw new Error('prompt store ref appears live and deleted.'); if(new Date(t.deletedAt).toISOString()!==t.deletedAt)throw new Error('tombstone has invalid deletedAt.');seen.add(t.ref); }
  return store;
}
...
export function mergePromptStore(oldStore, scanned, now = new Date()) {
  ...
  return validatePromptStore({ version: 1, generatedAt: now.toISOString(), sourceStatus: scanned.sourceStatus, prompts, tombstones: tombstones.sort((a,b) => a.ref.localeCompare(b.ref)) });
}

export function writePromptStore(cwd, store) {
  ...
  atomicWriteJson(join(cwd, PROMPT_STORE), store);
}
