...
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo() } = {}) {
  ...
  try {
    if (config.output.mode === 'site') {
      ...
      const redacted = redactor.deepRedact(bundle);
      // provenance.redactions is the true scrub count for this write (filled here,
      // after redaction). A derived number, so the fact-fence accepts it.
      if (redacted.provenance) redacted.provenance.redactions = redactor.count;
      result = await emitSite(redacted, config, { cwd }, { now: now.toISOString(), nowDate: now });
    } else {
