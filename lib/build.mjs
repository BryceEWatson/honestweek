...
import { verifyItems } from './git.mjs';
 ...
 */
export function assembleReportModel(items, config, verifiedIndex, week) {
...
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo() } = {}) {
  ...
  const model = assembleReportModel(items, config, verifiedIndex, week);
