...
import { citedShas, itemRepoLabel, verifyItems, repoMetricsInWindow } from './git.mjs';
...
import {
  emit, prepareEmission, prepareSiteEmission, writePreparedEmission,
} from './emit/index.mjs';
...
import { loadValidatedPromptLane } from './prompt-lane.mjs';
import { withPromptLock } from './prompt-lock.mjs';
import { assertNoDigestPending } from './digest-store.mjs';
import { assertNoDigestGroupCollision, isReservedDigestItem } from './digest-schema.mjs';
import {
  CARRY_GITIGNORE, deriveNextCarry, hashBytes, lifecycleTransactionNeeded, makeCarryPending, readCarry,
  removeCarryPending, writeCarry, writeCarryPending,
 ...
 */
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), promptLockHeld = false, transactionFs = {} } = {}) {
  if (!promptLockHeld) {
    let code;
    try {
      assertNoDigestPending(cwd);
      code = await withPromptLock(
        cwd,
        () => runBuild({ cwd, argv, now, io: { out: io.out, err: io.err, exit: (value) => value }, promptLockHeld: true, transactionFs }),
        { ensureIgnored: false },
      );
    } catch (err) {
    ...
    return code === 0 ? 0 : (io.exit(code) ?? code);
  }
  ...
  try {
    ...
    await recoverConfiguredCarry({
      cwd, config, hasGoals: existsSync(join(cwd, OBJECTIVES_FILE)), fs: transactionFs.recovery,
    });
  } catch (err) {
    io.err(`build: ABORTED: ${err.message}\nNo output was written.\n`);
    return io.exit(2) ?? 2;
  ...
  let items = Array.isArray(parsed) ? parsed : Array.isArray(parsed.items) ? parsed.items : [];
  if (items.some(isReservedDigestItem)) {
    io.err(`build: ABORTED: ${ITEMS_FILE} cannot contain reserved digest-lane item fields.\nNo output was written.\n`);
    return io.exit(2) ?? 2;
  }
  ...
  // Prompt-lane preflight is deliberately before every Git read and output
  // write. It re-resolves local transcript receipts and reconstructs the exact
  // canonical lane, so stale controls, source edits, or sidecar tampering abort.
  let promptLaneState;
  let priorCarry = { value: null, hash: null };
  let lifecycleActive = false;
  try {
    promptLaneState = await loadValidatedPromptLane({
      cwd, config, week, now,
      hasGoals: existsSync(join(cwd, OBJECTIVES_FILE)),
    });
    priorCarry = readCarry(cwd, config, { optional: true });
    if (priorCarry.value && promptLaneState.lane?.version !== 2) {
      throw new Error('canonical carry requires a prepared balanced digest lane.');
    }
    if (promptLaneState.lane?.version === 2) {
      lifecycleActive = lifecycleTransactionNeeded({
        review: promptLaneState.review, lane: promptLaneState.lane, priorCarry: priorCarry.value,
      });
    }
    assertNoDigestGroupCollision(items, promptLaneState.lane, projects, config.repos);
    items = [...items, ...promptLaneState.items];
  } catch (err) {
    io.err(`build: ABORTED: ${err.message}\nNo output was written.\n`);
    return io.exit(2) ?? 2;
  }
  ...
  // --- landed gate: `shipped` requires landed commits -----------------------
  // A `shipped` badge claims the work is on the default branch. verifyItems
  // re-derived that offline (local refs, never a fetch) as `landed` on each
  // verified commit. A real-but-unlanded commit keeps its receipt and
  // downgrades its item to the honest `in progress` — loudly, never silently.
  // When the repo has NO determinable default branch (`landed: null`) the
  // shipped claim is unverifiable, which aborts exactly like any other
  // unverifiable claim. Runs BEFORE assembly so every mode (post, changelog,
  // digest, report, page, site) sees the honest status.
  // Keyed by REPO **and** sha, never by sha alone: the same commit object can sit
  // in two configured repos (a fork and its upstream, a stale second clone) and be
  // landed in one but not the other. Under a sha-only key the last repo verified
  // overwrites the first one's verdict, which is exactly how an unlanded item
  // silently keeps `shipped`. The label is resolved with the same helper
  // verifyItems recorded it under, so a lookup cannot miss by spelling.
  const landedKey = (repoLabel, sha) => `${JSON.stringify(repoLabel ?? null)} ${sha}`;
  const landedByRepoSha = new Map(verified.map((v) => [landedKey(v.repoLabel, v.sha), v.landed]));
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    if (statusFor(item) !== 'shipped') continue;
    const shas = citedShas(item);
    if (shas.length === 0) continue;
    const ref = item.id ?? `item[${i}]`;
    const label = itemRepoLabel(item);
    // `true` is the ONLY verdict that sustains `shipped`. null (no determinable
    // default branch) and a missing entry (unreachable after verify-or-abort,
    // guarded so a future refactor fails closed) are both unverifiable.
    const verdictFor = (sha) => landedByRepoSha.get(landedKey(label, sha));
    const unknowable = shas.find((sha) => {
      const verdict = verdictFor(sha);
      return verdict === null || verdict === undefined;
    });
    if (unknowable !== undefined) {
      io.err(
        `build: ABORTED — item ${ref} claims "shipped" citing commit ${unknowable}, but repo ` +
          `${JSON.stringify(label)} has no determinable default branch (need origin/HEAD, a ` +
          `main/master branch, or a single local branch), so whether that commit landed cannot be ` +
          `verified offline.\nNo output was written.\n`
      );
      return io.exit(2) ?? 2;
    }
    const unlanded = shas.filter((sha) => verdictFor(sha) === false);
    if (unlanded.length === 0) continue;
    items[i] = { ...item, status: 'in progress' };
    io.err(
      `build: item ${ref} cites commit ${unlanded[0].slice(0, 7)} that has not landed on the default ` +
        `branch of repo ${JSON.stringify(label)} — "shipped" downgraded to "in progress" ` +
        `(real work, not landed yet).\n`
    );
  }
  ...
  try {
    if (config.output.mode === 'site') {
      ...
      preparedPrimary = await prepareSiteEmission(toEmit, config, { cwd }, { now: now.toISOString(), nowDate: now });
      ...
      preparedPrimary = prepareEmission(redactedForArchive, config, { cwd });
    } else {
      redactedForArchive = redactor.deepRedact(model);
      result = emit(redactedForArchive, config, { cwd });
    }
    if (preparedPrimary && lifecycleActive) {
      try {
        for (const entry of CARRY_GITIGNORE) ensureGitignore(cwd, entry);
      } catch (error) {
        const failure = new Error(`carry transaction did not start because private state could not be ignored (${error.message}); no output or carry was changed.`);
        failure.lifecycleTransaction = true;
        throw failure;
      }
      const nextCarry = deriveNextCarry({
        priorCarry: priorCarry.value, priorCarryHash: priorCarry.hash,
        review: promptLaneState.review, lane: promptLaneState.lane, week, config,
      });
      const outputBinding = {
        ...promptLaneState.lane.outputBinding,
        outputPath: resolve(preparedPrimary.path),
      };
      const pending = makeCarryPending({
        week, priorCarryHash: priorCarry.hash, carry: nextCarry,
        targetOutputHash: hashBytes(preparedPrimary.content), outputBinding, config,
      });
      try {
        writeCarryPending(cwd, pending, config, transactionFs.pending);
      } catch (error) {
        const failure = new Error(`carry transaction did not start because the pending write failed (${error.message}); no output or carry was changed.`);
        failure.lifecycleTransaction = true;
        throw failure;
      }
      try {
        result = writePreparedEmission(preparedPrimary, transactionFs.output);
      } catch (error) {
        try { removeCarryPending(cwd, transactionFs.removeAfterOutputFailure); }
        catch (removeError) {
          const failure = new Error(`primary output write failed (${error.message}) and the prepared carry marker remains (${removeError.message}); explicit pending discard is required.`);
          failure.lifecycleTransaction = true;
          throw failure;
        }
        const failure = new Error(`primary output write failed (${error.message}); prior output and carry were preserved.`);
        failure.lifecycleTransaction = true;
        throw failure;
      }
      try {
        writeCarryPending(cwd, { ...pending, phase: 'output-written' }, config, transactionFs.phase);
        writeCarry(cwd, nextCarry, config, transactionFs.carry);
        removeCarryPending(cwd, transactionFs.remove);
      } catch (error) {
        const failure = new Error(`carry transaction stopped after primary output installation (${error.message}); rerun any honestweek command to recover by hash.`);
        failure.lifecycleTransaction = true;
        throw failure;
      }
    } else if (preparedPrimary) {
      result = writePreparedEmission(preparedPrimary);
    }
    ...
    if (err && err.lifecycleTransaction) {
      io.err(`build: ABORTED: ${err.message}\n`);
      return io.exit(2) ?? 2;
    }
    io.err(`build: ${err.message}\n`);
