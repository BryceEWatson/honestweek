...
import {
  emit, emitSite, prepareEmission, prepareSiteEmission, resolvePrimaryOutputPath,
  writePreparedEmission,
} from './emit/index.mjs';
...
import { loadValidatedPromptLane, preflightPromptOutput } from './prompt-lane.mjs';
...
import {
  deriveNextCarry, hashBytes, lifecycleTransactionNeeded, makeCarryPending, readCarry,
  readCarryPending, recoverCarryPending, removeCarryPending, writeCarry, writeCarryPending,
...
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), promptLockHeld = false, transactionFs = {} } = {}) {
  ...
  try {
    if (readCarryPending(cwd, config, { optional: true })) {
      const outputBinding = await preflightPromptOutput({
        cwd, config, hasGoals: existsSync(join(cwd, OBJECTIVES_FILE)),
      });
  ...
  try {
    if (config.output.mode === 'site') {
      ...
      if (lifecycleActive) preparedPrimary = prepareEmission(redactedForArchive, config, { cwd });
      else result = emit(redactedForArchive, config, { cwd });
