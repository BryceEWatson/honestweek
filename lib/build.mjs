...
import { augmentSiteModel } from './site/derive.mjs';
...
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo() } = {}) {
  ...
  if (config.output.mode === 'site') {
    // Augment with the git/session-derived sections BEFORE redaction, so the
    // chart's per-day item titles are scrubbed by the same pass as the feed.
    const augmented = augmentSiteModel(model, { config, items, verified, verifiedIndex, week, now });
    toEmit = redactor.deepRedact(augmented);
    // provenance.redactions is the true scrub count for this write (filled here,
    // after redaction). It is a derived number, so the fact-fence accepts it.
    if (toEmit.provenance) toEmit.provenance.redactions = redactor.count;
  } else {
    toEmit = redactor.deepRedact(model);
  }

  let result;
  try {
    result = emit(toEmit, config, { cwd });
  } catch (err) {
