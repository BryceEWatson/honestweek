 ...
 */
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), promptLockHeld = false } = {}) {
  if (!promptLockHeld) {
    let code;
    try {
      ...
      code = await withPromptLock(
        cwd,
        () => runBuild({ cwd, argv, now, io: { out: io.out, err: io.err, exit: (value) => value }, promptLockHeld: true }),
        { ensureIgnored: false },
      );
    } catch (err) {
  ...
  try {
    const promptLane = await loadValidatedPromptLane({
      cwd, config, week, now,
      hasGoals: existsSync(join(cwd, OBJECTIVES_FILE)),
    });
    assertNoDigestGroupCollision(items, promptLane.lane, projects, config.repos);
