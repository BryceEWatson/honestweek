...
import { citedShas, verifyItems, repoMetricsInWindow } from './git.mjs';
...
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo(), promptLockHeld = false, transactionFs = {} } = {}) {
  ...
  const landedBySha = new Map(verified.map((v) => [v.sha, v.landed]));
  for (let i = 0; i < items.length; i++) {
    ...
    const unknowable = shas.find((sha) => landedBySha.get(sha) === null);
    if (unknowable !== undefined) {
      io.err(
        `build: ABORTED — item ${ref} claims "shipped" citing commit ${unknowable}, but repo ` +
          `${JSON.stringify(item.repo)} has no determinable default branch (need origin/HEAD, a ` +
          `main/master branch, or a single local branch), so whether that commit landed cannot be ` +
          `verified offline.\nNo output was written.\n`
      );
      return io.exit(2) ?? 2;
    }
    const unlanded = shas.filter((sha) => landedBySha.get(sha) === false);
    ...
    io.err(
      `build: item ${ref} cites commit ${unlanded[0].slice(0, 7)} that has not landed on the default ` +
        `branch of repo ${JSON.stringify(item.repo)} — "shipped" downgraded to "in progress" ` +
