...
export async function runBuild({ cwd = process.cwd(), argv = [], now = new Date(), io = defaultIo() } = {}) {
  ...
  // existing consumer is unchanged. It scans the AUTHORED inputs (items prose +
  // curated content/projects), never evidence snippets/receipts — and the authored
  // prose is identical across output modes, so checking the source once covers all.
  ...
  try {
    if (config.output.mode === 'site') {
      ...
      const pageModel = buildPageModel({ items, config, verifiedIndex, week, chart, metricsByLabel, content, hasGoals: !!goalsRegistry });
