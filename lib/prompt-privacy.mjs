...
function selectedOps(raw, config, found = candidates(raw, config)) {
  ...
  const sorted = found.slice().sort((a, b) => a.start - b.start || rank.get(a.detector) - rank.get(b.detector) || (b.end - b.start) - (a.end - a.start));
  const out = [];
  for (const op of sorted) if (!out.some((x) => op.start < x.end && op.end > x.start)) out.push(op);
