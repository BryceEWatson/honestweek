...
/**
 * emitSite(reportModel, config, cwd) -> { path, mode, bytes, items }
 * Loads the committed site adapter, resolves it against the verified, derived-
 * augmented model (renderSite runs the fact-fence; a violation throws
 * FactFenceError, which the build maps to exit 2), and writes the JSON artifact to
 * the adapter's `artifact` path (relative to the target root = cwd). Local write
 * only — the target's existing PR flow is the publish gate.
 */
function emitSite(reportModel, config, cwd) {
  ...
  const artifact = renderSite(reportModel, adapter); // validates + fact-fences (may throw)
  const outPath = isAbsolute(adapter.artifact) ? adapter.artifact : join(cwd, adapter.artifact);
  ...
  return { path: outPath, mode: 'site', bytes: Buffer.byteLength(content, 'utf8'), items: countItems(reportModel) };
}
...
export function emit(reportModel, config, { cwd = process.cwd() } = {}) {
  ...
  if (mode === 'site') return emitSite(reportModel, config, cwd);
