...
export function buildPageModel({ items = [], config = {}, verifiedIndex = new Map(), week = {}, chart = null, metricsByLabel = new Map(), sessions = null, content = null, hasGoals = false } = {}) {
  ...
  const groups = [...groupsByLabel.values()]
    .map((g) => {
      ...
      // Entries-derived floor (cross-cwd reconciliation): a group already shown as a commit/session
      // source is lifted to at least the distinct days its own dated items span, so its "active D/N
      // days" can never undercount the entries rendered beneath it (matches the site engine's
      // deriveProjectStats floor). Distinct dates among this group's items — undated items contribute
      // nothing (max, so a null-dated item never lowers the count).
      const entryDays = new Set(g.items.map((it) => it.date).filter(Boolean)).size;
      // max(commit-days, session-days, entry-days): a display/session-only repo (no git metric) shows the
      // days it had interactive sessions instead of a blank. null ONLY when there's no git metric AND no
      // sessions (a never-git-read repo with no sessions shows no fabricated 0-for-real; a pure dated
      // group makes no active-days claim to contradict), so the entry-days floor is applied only once
      // there is already a commit- or session-day signal to reconcile.
      const activeDays = git == null && sDays === 0 ? null : Math.max(git ? git.activeDays : 0, sDays, entryDays);
