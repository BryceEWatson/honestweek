...
export function buildPageModel({ items = [], config = {}, verifiedIndex = new Map(), week = {}, chart = null, metricsByLabel = new Map(), sessions = null, content = null, hasGoals = false } = {}) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const status = isPrompt ? '' : STATUSES.includes(item.status) ? item.status : 'designed, not proven';
    ...
    const view = {
      ...
      kind: isPrompt ? 'prompt' : 'work',
      sessionReceipt: isPrompt ? item.receipt ?? null : null,
      snippets: isPrompt && Array.isArray(item.snippets) ? item.snippets : [],
    };
    const label = isPrompt ? 'Prompt highlights' : item.repo;
    if (isPrompt || (label && roleByLabel.has(label))) {
      if (!groupsByLabel.has(label)) groupsByLabel.set(label, { label, role: isPrompt ? 'prompt' : roleByLabel.get(label), items: [] });
  ...
  const groups = [...groupsByLabel.values()]
    .map((g) => {
      ...
      const sDays = (sessions?.days ?? []).filter((d) => (d.byProject?.[g.label] || 0) > 0).length;
      ...
      const activeDays = git == null && sDays === 0 ? null : activeSignal > 0 ? Math.max(activeSignal, entryDays) : activeSignal;
      ...
      const itemsHtml = g.items
        .map((it) => {
          const badge = BADGE_CLASS[it.status]
            ...
            : '';
          const drawer = it.kind === 'prompt' && it.sessionReceipt
...
<div class="wl-foot"><span class="wl-mono" style="color:var(--mute)">honestweek build</span> &nbsp;${(m.groups || []).some((g) => g.label === 'Prompt highlights') ? 'Every line carries a source receipt; work claims retain their git verification.' : 'Every line carries a status badge and a git receipt; every number is re-derived from git.'} Generated locally; nothing here is published until you publish it.</div>
</section>
