 ...
 * `sessions.days[].byProject[label]`), so a display/session-only repo shows the days
 * it had interactive sessions; `sessions` is optional (absent -> git-only, unchanged).
 */
export function buildPageModel({ items = [], config = {}, verifiedIndex = new Map(), week = {}, chart = null, metricsByLabel = new Map(), sessions = null, content = null, hasGoals = false } = {}) {
  ...
  const groups = [...groupsByLabel.values()]
    .map((g) => {
      ...
      // max(commit-days, session-days): a display/session-only repo (no git metric) shows the days it
      // had interactive sessions instead of a blank. null ONLY when there's no git metric AND no
      // sessions, so a never-git-read repo with no sessions still shows no fabricated 0-for-real.
      const activeDays = git == null && sDays === 0 ? null : Math.max(git ? git.activeDays : 0, sDays);
