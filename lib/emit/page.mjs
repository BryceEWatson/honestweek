...
//   - every NUMBER on the page is honestweek's OWN deterministic derivation, never
//     re-emitted from an untrusted source. Chart totals and per-project commits are
//     git-derived; per-project "active days" is max(commit-active days, session-active
//     days), where the session-active count is counted deterministically from the local
//     session logs — a weaker, session-derived evidence class than a git commit, but
//     never authored (a display/session-only repo shows the days it had sessions instead
//     of a blank). NB: page mode has no fact-fence (the numeric fence guards site/
//     transform mode only), so this honesty rests on the derivation itself, not a fence;
...
import { DIGEST_GROUPS, DIGEST_KINDS } from '../digest-schema.mjs';
...
// The `shipped` wording states exactly what build enforces: the cited commits
// resolved, are yours, and are reachable from the repo's default branch (the
// landed gate) — unlanded work can never carry this legend.
const LEGEND = [
  { key: 'shipped', label: 'shipped: built, landed on the default branch, verified', color: '#c8f751' },
 ...
 *  Escapes all five markup-significant characters (incl. both quote styles), so a
 *  value is safe in text AND in single- or double-quoted attribute contexts. */
...
/**
 * buildPageModel({ items, config, verifiedIndex, week, chart, metricsByLabel, sessions, content })
 ...
 * A group's `activeDays` is max(git commit-active days, session-active days from
 * `sessions.days[].byProject[label]`, and — once there is any commit/session signal —
 * the distinct days the group's own dated items span), so a display/session-only repo
 * shows the days it had interactive sessions and no group renders "active 1 day" above
 * 2 rows dated on 2 days; `sessions` is optional (absent -> git+entry-days, unchanged
 * when there are no cross-cwd entries).
 */
export function buildPageModel({ items = [], config = {}, verifiedIndex = new Map(), week = {}, chart = null, metricsByLabel = new Map(), sessions = null, content = null, hasGoals = false } = {}) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const isV2Digest = Array.isArray(item.receipts) && DIGEST_KINDS.includes(item.kind) && item.publicDisposition === 'automatic-safe' &&
      DIGEST_GROUPS.includes(item.project) && item.status === '' && Array.isArray(item.snippets);
    const isV1Prompt = !isV2Digest && item.kind === 'prompt';
    const isSessionMaterial = isV2Digest || isV1Prompt;
    const status = isSessionMaterial ? '' : STATUSES.includes(item.status) ? item.status : 'designed, not proven';
    const summary = item.summary ?? item.text ?? '';
    const date = v ? String(v.dateISO).slice(0, 10) : item.date ?? null;
    const view = {
      // A stable, anchor-safe item id: report.html renders it as a DOM id and the
      // goals page deep-links to report.html#<id>. Synthesized when the item carries
      // none, so the two pages always agree on the anchor.
      id: item.id != null && String(item.id).trim() ? String(item.id) : `e${i}`,
      // Carried (not rendered) so the goals lens can resolve an explicit per-item
      // objectiveId override before falling back to the repo->goal mapping.
      ...
      kind: isV2Digest ? 'session' : isV1Prompt ? 'prompt' : 'work',
      sessionReceipt: isSessionMaterial ? item.receipt ?? null : null,
      snippets: isSessionMaterial ? item.snippets : [],
    };
    const label = isV2Digest ? item.project : isV1Prompt ? 'Prompt highlights' : item.repo;
    const groupKey = isV2Digest ? `digest:${item.category}` : label;
    if (isSessionMaterial || (label && roleByLabel.has(label))) {
      if (!groupsByLabel.has(groupKey)) groupsByLabel.set(groupKey, { label, role: isV2Digest ? 'digest' : isV1Prompt ? 'prompt' : roleByLabel.get(label), items: [] });
      groupsByLabel.get(groupKey).items.push(view);
    } else {
      loose.push(view);
    }
  }
  ...
  const groups = [...groupsByLabel.values()]
    .map((g) => {
      const isDigest = g.role === 'digest';
      const git = isDigest ? null : metricsByLabel.get(g.label) || null;
      // Session-active days for this group's label: the session bundle buckets by config repo-label
      // (or the literal 'other' for the catch-all group), the same key page mode groups by. Counted
      // over the SAME week grid as the chart (sessions.days.length === chart.windowDays), so it can
      // never exceed the rendered "/windowDays" denominator.
      const sDays = isDigest ? 0 : (sessions?.days ?? []).filter((d) => (d.byProject?.[g.label] || 0) > 0).length;
      // The commit/session signal for this group: the days git or the session bundle already prove active.
      // A readable-but-quiet repo yields git = { commits: 0, activeDays: 0 } (non-null — repoMetricsInWindow
      // returns null ONLY for an unreadable repo), so `git != null` is NOT a proxy for "has activity".
      const activeSignal = Math.max(git ? git.activeDays : 0, sDays);
      // Entries-derived floor (cross-cwd reconciliation): a group already shown as a commit/session source
      // (activeSignal > 0) is lifted to at least the distinct days its own dated items span, so its
      // "active D/N days" can never undercount the entries rendered beneath it. Distinct dates among this
      // group's items — undated items contribute nothing (max, so a null-dated item never lowers the count).
      const entryDays = new Set(g.items.map((it) => it.date).filter(Boolean)).size;
      // Gate the floor on activeSignal > 0, EXACTLY mirroring the site engine's deriveProjectStats
      // (derive.mjs) — otherwise a readable-but-quiet repo (git non-null, activeSignal 0) would floor here
      // while the site engine does not, and page mode and site mode would disagree on the same project's
      // active-day count. null ONLY when there's no git metric AND no sessions (a never-git-read repo with
      // no sessions shows no fabricated 0-for-real; a pure dated group makes no active-days claim to
      // contradict). A signalled group with no entries keeps its commit/session count (max, entryDays 0).
      const activeDays = isDigest || (git == null && sDays === 0) ? null : activeSignal > 0 ? Math.max(activeSignal, entryDays) : activeSignal;
      // items most-recent-first
      if (g.role !== 'digest') g.items.sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
      return {
        label: g.label,
        _digest: g.role === 'digest',
        metrics: {
    ...
    .sort((a, b) => {
      const aDigest = a._digest ? DIGEST_GROUPS.indexOf(a.label) : -1;
      const bDigest = b._digest ? DIGEST_GROUPS.indexOf(b.label) : -1;
      if (a._digest || b._digest) return !a._digest ? -1 : !b._digest ? 1 : aDigest - bDigest;
      return String(b.mostRecent).localeCompare(String(a.mostRecent));
    });
  for (const group of groups) delete group._digest;
  ...
  return {
    ...
    // True when a goals.html will be emitted beside this report (opt-in registry
    // present): render then shows the "by goal" cross-link. Default single-page.
    hasGoals: !!hasGoals,
...
function chartBars(chart) {
  ...
  // Coerce to finite numbers so a malformed chart can never inject non-numeric
  // bytes into the style="height:..%"/--a attribute (defense-in-depth: the build
  // path always passes deriveChart's finite numbers).
  const max = Math.max(1, Number(chart.max) || 0);
  ...
  return days.map((d, i) => {
    ...
    const label = total === 0 ? `${head}: no commits` : `${head}: ${total} commit${total === 1 ? '' : 's'}${breakdown ? ` (${breakdown})` : ''}`;
    return {
...
.wl-bygoal{display:inline-flex;align-items:center;gap:6px;margin:14px 0 0;font-size:11px;color:var(--link);text-decoration:none;font-family:"JetBrains Mono",ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
.wl-bygoal:hover{text-decoration:underline}
      ...
      const itemsHtml = g.items
        .map((it) => {
          const badge = BADGE_CLASS[it.status]
            ...
            : '';
          const drawer = (it.kind === 'session' || it.kind === 'prompt') && it.sessionReceipt
            ? `<details class="wl-dig"><summary class="wl-mono"><span class="wl-chev" aria-hidden="true">&#9656;</span><span>source receipt</span></summary>${it.snippets.map((s) => `<div class="wl-snip"><div class="wl-snipkind wl-mono">${esc(s.kind)} · ${esc(s.source)}</div><div class="wl-sniptext">${esc(s.text)}</div></div>`).join('')}</details>`
            : it.receipt
            ...
            : '';
          // id is the deep-link target for goals.html (report.html#<id>); scroll-margin
          // keeps the anchored card clear of the top when jumped to.
          return `<div class="wl-item${it.tier !== 'routine' ? ' is-spot' : ''}" id="${esc(it.id)}" style="scroll-margin-top:16px" data-date="${esc(it.date)}"><div class="wl-item-meta">${badge}<span class="wl-date wl-mono">${esc(it.dateLabel || '')}</span></div><h3 class="wl-serif wl-title">${esc(it.title)}</h3><p class="wl-summary">${esc(it.summary)}</p>${drawer}</div>`;
        })
        .join('');
...
<div class="wl-intro"><h1 class="wl-serif wl-headline">${esc(m.headline || '')}</h1><ul class="wl-legend wl-mono" aria-label="Status key">${legendHtml}</ul>${m.hasGoals ? `<a class="wl-bygoal" href="goals.html">see the same work by goal <span aria-hidden="true">&rarr;</span></a>` : ''}</div>
<div class="wl-feed">${feedInner}</div>
<div class="wl-foot"><span class="wl-mono" style="color:var(--mute)">honestweek build</span> &nbsp;${(m.groups || []).some((g) => g.label === 'Prompt highlights' || g.items.some((item) => item.kind === 'session')) ? 'Every line carries a source receipt; work claims retain their git verification.' : 'Every line carries a status badge and a git receipt; every number is re-derived from git.'} Generated locally; nothing here is published until you publish it.</div>
</section>
