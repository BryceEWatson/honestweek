...
/** Format git-derived activity metrics as a compact line, e.g.
 *  "4 commits · 3 active days · 5 entries". `commits`/`activeDays` are git-derived
 *  (or absent when a repo was unreadable — never fabricated); `entries` is the
 *  render-count of items shown. Returns '' for absent metrics. */
export function formatMetrics(metrics) {
  if (!metrics) return '';
  const parts = [];
  if (metrics.commits != null) parts.push(`${metrics.commits} commit${metrics.commits === 1 ? '' : 's'}`);
  if (metrics.activeDays != null) parts.push(`${metrics.activeDays} active day${metrics.activeDays === 1 ? '' : 's'}`);
  if (metrics.entries != null) parts.push(`${metrics.entries} ${metrics.entries === 1 ? 'entry' : 'entries'}`);
  return parts.join(' · ');
}
