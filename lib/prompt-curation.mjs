...
function tokens(text) { return text.normalize('NFKC').toLowerCase().replace(/\[redacted:(?:email|secret|path|term|account)\]/g,' redacted ').replace(/[^\p{L}\p{N}]+/gu,' ').trim().split(/\s+/).filter((x) => x && !filler.has(x)); }
function recurs(a,b) { if (a.sessionKey === b.sessionKey) return false; const A=new Set(tokens(a.text)), B=new Set(tokens(b.text)); if(A.size<4||B.size<4)return false; let n=0; for(const x of A)if(B.has(x))n++; return n/(A.size+B.size-n)>=.75; }
