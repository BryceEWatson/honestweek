...
function preselectionDecision(p, config) {
  if(p.state==='hidden')return 'hidden';
  if(p.isPrivate)return 'private-source';
  if(assessPublicRendition(p.text,config)==='high')return 'high-risk';
  if(p.truncated||p.changedPercent>config.privacy.publicRenditions.maxAutomaticChangedPercent||p.rawDetectors.includes('capitalized-unknown'))return 'needs-approval';
  return null;
}
...
export function evaluatePrompts(store,config,week){
  ...
  const recurrenceSources=live.filter((p)=>preselectionDecision(p,config)===null);
  const evaluated=live.map((p)=>{const codes=signalsFor(p,recurrenceSources);return{p,codes,score:codes.reduce((n,c)=>n+(weights[WEIGHT_KEYS[c]]??0),0)};});
  const eligible=[];
  for(const e of evaluated){
    const p=e.p; let why=preselectionDecision(p,config);
    if(!why&&p.state!=='kept'&&(e.score<floor||!e.codes.some((c)=>c==='recurs'||c==='observed-verification')))why='below-automatic-floor';
