...
export async function runPrompts({cwd=process.cwd(),argv=[],now=new Date(),io=ioDefault(),roots}={}){
  ...
  // Help before config, and anywhere in argv: `prompts sync --help` used to run
  // a live sync. Asking what a command does must not require a config or do work.
  if(!command||argv.some((a)=>a==='-h'||a==='--help')){io.out('Usage: honestweek prompts <sync|list|source|review|keep|hide|delete|curate> [options]\n');return 0;}
