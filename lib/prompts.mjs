...
function mutate(cwd,store,ref,state,now){const p=uniquePrompt(store,ref);if(state==='deleted'){store.prompts=store.prompts.filter((x)=>x.ref!==p.ref);store.tombstones.push({ref:p.ref,refCanonical:p.refCanonical,source:p.source,sessionKey:p.sessionKey,turn:p.turn,deletedAt:now.toISOString()});store.tombstones.sort((a,b)=>a.ref.localeCompare(b.ref));}else p.state=state;store.generatedAt=now.toISOString();writePromptStore(cwd,store);return p;}
...
export async function runPrompts({cwd=process.cwd(),argv=[],now=new Date(),io=ioDefault(),roots}={}){
  ...
  try{
    if(command==='sync'){
      const store=await withPromptLock(cwd,()=>sync(cwd,config,rest,now,roots??resolvePromptRoots()));
    ...
    if(command==='curate'){
      ...
      const w=weekFor(config,rest,now);const lane=await withPromptLock(cwd,async()=>{const fresh=await sync(cwd,config,rest,now,roots??resolvePromptRoots());privateInboxUpdated=true;if(Object.values(fresh.sourceStatus).every((s)=>s.state==='absent'))throw new Error('no Claude Code or Codex prompt source is available; prompt lane not updated.');const value=curatePrompts(fresh,config,{start:w.start,end:w.end},now,{outputBinding});writePromptLane(cwd,value);return value;});
      const counts=Object.entries(lane.withheld).map(([k,v])=>`${k}=${v}`).join(', ');const keeps=lane.items.filter((x)=>x.curationState==='kept').length;io.out(`prompts curate: selected ${lane.items.length}; ${counts}. Automatic cap ${lane.policy.automaticCap}.${keeps>0&&lane.items.length>lane.policy.automaticCap?' Explicit keeps exceeded the automatic target.':''} Claude Code ${lane.sourceStatus['claude-code'].state}; Codex ${lane.sourceStatus.codex.state}.\n`);return 0;
    }
    if(command==='keep'||command==='hide'||command==='delete')return await withPromptLock(cwd,async()=>{
      const store=readPromptStore(cwd);
      if(command==='delete'){if(!rest.includes('--yes'))throw new Error('delete requires --yes.');const p=mutate(cwd,store,rest[0],'deleted',now);io.out(`prompts delete: removed private prompt ${p.ref.slice(0,12)} and left a no-text tombstone. This cannot recall ${PROMPT_LANE} or ${config.output.file??'the built page'}. Run honestweek prompts curate, honestweek validate, then honestweek build; remove the local output now if needed.\n`);return 0;}
