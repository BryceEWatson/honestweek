...
/** The real UTC instant at local 00:00 for a YYYY-MM-DD date in an IANA zone. */
export function localMidnightInstant(dateKey, timezone) {
  const m=/^(\d{4})-(\d{2})-(\d{2})$/.exec(dateKey);
  if(!m)throw new WeekResolutionError(`invalid local date ${JSON.stringify(dateKey)}.`);
  ...
  let guess=target;
  for(let i=0;i<3;i++){
    const parts=fmt.formatToParts(new Date(guess));const get=(type)=>Number(parts.find((p)=>p.type===type)?.value);
...
export function localDateRangeInstants(startKey,endKey,timezone){
  const endDate=new Date(`${endKey}T00:00:00.000Z`);if(Number.isNaN(endDate.getTime()))throw new WeekResolutionError('invalid local end date.');endDate.setUTCDate(endDate.getUTCDate()+1);
  return{start:localMidnightInstant(startKey,timezone),endExclusive:localMidnightInstant(endDate.toISOString().slice(0,10),timezone)};
}
