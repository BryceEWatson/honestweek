...
export function defaultBranchRefs(repoPath) {
  ...
  const candidates = [];
  if (name) {
    candidates.push(`refs/remotes/origin/${name}`, `refs/heads/${name}`);
  } else {
    for (const conventional of ['main', 'master']) {
      const pair = [`refs/remotes/origin/${conventional}`, `refs/heads/${conventional}`]
  ...
  return candidates.filter((ref) => refResolves(repoPath, ref));
}
...
export function commitReachableFrom(repoPath, sha, refs) {
  return (Array.isArray(refs) ? refs : []).some(
    (ref) => runGit(repoPath, ['merge-base', '--is-ancestor', sha, ref]).ok
  );
}
