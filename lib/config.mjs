...
export function normalizeConfig(raw, { configDir = process.cwd() } = {}) {
  ...
  const curation = {
    ...
    retentionWeeks: int(rawCuration?.retentionWeeks, 12, 'curation.retentionWeeks', 1, 52),
    automaticCarryWeeks: int(rawCuration?.automaticCarryWeeks, 2, 'curation.automaticCarryWeeks', 0, 8),
  };
  ...
  const privacy = { publicRenditions: {
    enabled,
    maxAutomaticChangedPercent: int(rawPublic?.maxAutomaticChangedPercent, 20, 'privacy.publicRenditions.maxAutomaticChangedPercent', 0, 100),
