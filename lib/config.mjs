 ...
 *  `site` integrates a verified report into a target website's data artifact —
 *  its write path comes from the committed adapter (output.adapter), not a fixed
 *  default file, so it has no entry in DEFAULT_OUTPUT_FILES. */
export const OUTPUT_MODES = ['post', 'changelog', 'digest', 'report', 'site'];
...
export function normalizeConfig(raw, { configDir = process.cwd() } = {}) {
  ...
  return {
    ...
    repos,
    redaction,
    output: { mode, file, archive, archiveDir, ...(adapter !== undefined ? { adapter } : {}) },
  };
}
