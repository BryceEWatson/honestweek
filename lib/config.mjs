...
export function normalizeConfig(raw, { configDir = process.cwd() } = {}) {
  ...
  // --- mine (optional; drives `honestweek mine`) ---
  // Absent -> mining still runs, with defaults and no destination schema. Everything
  // here is about the DESTINATION, which honestweek deliberately knows nothing else
  // about: where drafts go, what frontmatter that site uses, which repos are yours,
  // and which errors it has already covered.
  const rawMine = raw.mine;
  if (rawMine !== undefined && !isPlainObject(rawMine)) {
    fail('"mine" must be an object when present.');
  }
  let mine;
  if (rawMine !== undefined) {
    const rawDraft = rawMine.draft;
    if (rawDraft !== undefined && !isPlainObject(rawDraft)) {
      fail('"mine.draft" must be an object when present.');
    }
    if (rawDraft?.dir !== undefined && (typeof rawDraft.dir !== 'string' || rawDraft.dir.length === 0)) {
      fail('"mine.draft.dir" must be a non-empty string when present.');
    }
    if (rawDraft?.frontmatter !== undefined && !isPlainObject(rawDraft.frontmatter)) {
      fail('"mine.draft.frontmatter" must be an object when present.');
    }
    if (rawMine.ledger !== undefined && (typeof rawMine.ledger !== 'string' || rawMine.ledger.length === 0)) {
      fail('"mine.ledger" must be a non-empty string when present.');
    }
    // Shape-validated like identity.authorEmails, not merely "an array of strings".
    // An entry without an "owner/name" shape was accepted here and silently discarded
    // at use, which turns issues on the user's OWN repo into third-party evidence —
    // the exact discriminator the miner is built around.
    const ownRepos = normalizeStringArray(rawMine.ownRepos, 'mine.ownRepos');
    ownRepos.forEach((slug, i) => {
      if (!/^[\w.-]+\/[\w.-]+$/.test(slug)) {
        fail(`"mine.ownRepos[${i}]" must be "owner/name" (got ${JSON.stringify(slug)}).`);
      }
    });
    mine = {
      ...(rawMine.ledger !== undefined ? { ledger: rawMine.ledger } : {}),
      ownRepos,
      publishedErrorStrings: normalizeStringArray(rawMine.publishedErrorStrings, 'mine.publishedErrorStrings'),
      ...(rawDraft !== undefined
        ? { draft: { ...(rawDraft.dir !== undefined ? { dir: rawDraft.dir } : {}), ...(rawDraft.frontmatter !== undefined ? { frontmatter: rawDraft.frontmatter } : {}) } }
        : {}),
    };
  }

  // --- curation + public rendition policy (local prompt lane) ---
  const rawCuration = raw.curation;
  if (rawCuration !== undefined && !isPlainObject(rawCuration)) fail('"curation" must be an object when present.');
  const allowedCuration = new Set(['maxItems','automaticMinScore','categoryCaps','weights','retentionWeeks','automaticCarryWeeks']);
  const unknownCuration = Object.keys(rawCuration ?? {}).filter((k) => !allowedCuration.has(k));
  if (unknownCuration.length) fail(`"curation" contains unknown key ${JSON.stringify(unknownCuration[0])}.`);
  const int = (value, dflt, field, min, max) => {
    const v = value ?? dflt;
    if (!Number.isInteger(v) || v < min || v > max) fail(`"${field}" must be an integer from ${min} to ${max}.`);
    return v;
  };
  const defaultCaps = { prompts: 2, ideas: 2, techniques: 3, decisions: 2, reversals: 1, nextSteps: 2 };
  const defaultWeights = {
    'decision-or-reversal': 3, 'observed-verification': 2, recurs: 2,
    'unresolved-next-step': 1, 'follow-on-correction': 1,
    'positive-feedback': 2, 'negative-feedback': -2,
    'decision-request': 1, 'reversal-request': 1, 'next-step-request': 1,
  };
  const rawCaps = rawCuration?.categoryCaps;
  if (rawCaps !== undefined && !isPlainObject(rawCaps)) fail('"curation.categoryCaps" must be an object when present.');
  const rawWeights = rawCuration?.weights;
  if (rawWeights !== undefined && !isPlainObject(rawWeights)) fail('"curation.weights" must be an object when present.');
  const unknownCaps = Object.keys(rawCaps ?? {}).filter((k) => !(k in defaultCaps));
  if (unknownCaps.length) fail(`"curation.categoryCaps" contains unknown key ${JSON.stringify(unknownCaps[0])}.`);
  const unknownWeights = Object.keys(rawWeights ?? {}).filter((k) => !(k in defaultWeights));
  if (unknownWeights.length) fail(`"curation.weights" contains unknown key ${JSON.stringify(unknownWeights[0])}.`);
  const curation = {
    maxItems: int(rawCuration?.maxItems, 12, 'curation.maxItems', 1, 50),
    automaticMinScore: int(rawCuration?.automaticMinScore, 2, 'curation.automaticMinScore', -10, 10),
    categoryCaps: Object.fromEntries(Object.entries(defaultCaps).map(([k,v]) => [k, int(rawCaps?.[k], v, `curation.categoryCaps.${k}`, 0, 20)])),
    weights: Object.fromEntries(Object.entries(defaultWeights).map(([k,v]) => [k, int(rawWeights?.[k], v, `curation.weights.${k}`, -10, 10)])),
    retentionWeeks: int(rawCuration?.retentionWeeks, 12, 'curation.retentionWeeks', 1, 12),
    automaticCarryWeeks: int(rawCuration?.automaticCarryWeeks, 2, 'curation.automaticCarryWeeks', 0, 2),
  };

  const rawPrivacy = raw.privacy;
  if (rawPrivacy !== undefined && !isPlainObject(rawPrivacy)) fail('"privacy" must be an object when present.');
  if (Object.keys(rawPrivacy ?? {}).some((k) => k !== 'publicRenditions')) fail('"privacy" contains an unknown key.');
  const rawPublic = rawPrivacy?.publicRenditions;
  if (rawPublic !== undefined && !isPlainObject(rawPublic)) fail('"privacy.publicRenditions" must be an object when present.');
  const allowedPublic = new Set(['enabled','maxAutomaticChangedPercent','generalizationMappings','neverPublicTerms']);
  if (Object.keys(rawPublic ?? {}).some((k) => !allowedPublic.has(k))) fail('"privacy.publicRenditions" contains an unknown key.');
  const enabled = rawPublic?.enabled ?? true;
  if (typeof enabled !== 'boolean') fail('"privacy.publicRenditions.enabled" must be a boolean.');
  const mappings = rawPublic?.generalizationMappings ?? {};
  if (!isPlainObject(mappings)) fail('"privacy.publicRenditions.generalizationMappings" must be an object.');
  if (Object.keys(mappings).length) fail('"privacy.publicRenditions.generalizationMappings" is not supported in Slice 1; use canonical redaction terms.');
  const privacy = { publicRenditions: {
    enabled,
    maxAutomaticChangedPercent: int(rawPublic?.maxAutomaticChangedPercent, 20, 'privacy.publicRenditions.maxAutomaticChangedPercent', 0, 20),
    generalizationMappings: {},
    neverPublicTerms: normalizeStringArray(rawPublic?.neverPublicTerms, 'privacy.publicRenditions.neverPublicTerms'),
  } };
