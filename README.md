...
| `output.adapter` | **Required for `site` mode only**: path to the committed `honestweek.site.json` adapter (resolved like a repo path). It maps the verified model onto the site's data artifact; the artifact's own write path lives in the adapter. |
