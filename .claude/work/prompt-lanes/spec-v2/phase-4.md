...
`lib/build.mjs:353` and the lane-absent stdout remain unchanged. `--no-lanes` skips the Phase 3
...
`lib/build.mjs:294`. Assert that same D45 value on every final row, including unchanged current
...
block at `lib/build.mjs:547`. (D5, D10, D11, D12, D45, D57, D64, D65, D69)
...
at `lib/build.mjs:563` remains byte-identical only when all three raw sources are absent, and the
...
`ensureGitignore` helper at `lib/init.mjs:123`, and add `honestweek.forward-index.json` to the
...
index, and preserves the exact lane-absent stdout bytes rooted at `lib/build.mjs:563`. (R1, R8,
R12)
...
current row, forcing the fallback branch reached by the call at `lib/build.mjs:294`. Week 2 renders
