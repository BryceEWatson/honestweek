...
are `lib/claude-adapter.mjs:529` and `lib/handoffs.mjs:103`. R30, not a consumer, builds the two
...
`lib/validate.mjs:124`. These are positive dishonesty hits in validate and build, reported by row id
with no matched label or term echoed. Do not reuse work-item `itemRepoLabel` alone: the current
field-only check begins at `lib/validate.mjs:114` and cannot see a label in lane prose.
(D7, D10, D32)
...
`lib/build.mjs:298`, so a lane violation reports its D67 technique id and can never be mislabeled as
...
`techniques` beside the current work-item extraction at `lib/validate.mjs:165`. When a lane is
...
spread. In markdown modes, copy the base model returned at `lib/build.mjs:285`, conditionally attach
the derived `techniques`, then pass that object to the redaction call currently at
`lib/build.mjs:405`. In page mode, conditionally attach after `buildPageModel` returns at
`lib/build.mjs:385` and before the redaction call at `lib/build.mjs:386`. Pass the untouched base
model into `augmentSiteModel` at `lib/build.mjs:355`, and assert both its input and return have
...
must reproduce the current `LEGEND` and `BADGE_CLASS` bytes at `lib/emit/page.mjs:32` and
`lib/emit/page.mjs:37`; Lane A instantiates it with `TECHNIQUE_GRADES`; Phase 3 may only instantiate
...
`lib/emit/digest.mjs:25`, page foot at `lib/emit/page.mjs:391`, and README sample line at
`README.md:127`. (D6, D16, D55, D60)
...
`lib/emit/report.mjs:47`, `lib/emit/post.mjs:16`, and `lib/emit/page.mjs:369`. (D6, D25, D55)
...
stdout branch at `lib/build.mjs:438` byte for byte, including its optional goals and archive suffixes:
...
This replaces the current AGENTS wording at `AGENTS.md:40` and README wording at `README.md:285`
