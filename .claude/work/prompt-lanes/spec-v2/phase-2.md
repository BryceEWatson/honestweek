...
`lib/validate.mjs:127`. These are positive dishonesty hits in validate and build, reported by row id
with no matched label or term echoed. Do not reuse work-item `itemRepoLabel` alone: the current
field-only check begins at `lib/validate.mjs:117` and cannot see a label in lane prose.
(D7, D10, D32)
...
`lib/build.mjs:328`, so a lane violation reports its D67 technique id and can never be mislabeled as
...
`techniques` beside the current work-item extraction at `lib/validate.mjs:182`. When a lane is
...
spread. In markdown modes, copy the base model returned at `lib/build.mjs:315`, conditionally attach
the derived `techniques`, then pass that object to the redaction call currently at
`lib/build.mjs:435`. In page mode, conditionally attach after `buildPageModel` returns at
`lib/build.mjs:415` and before the redaction call at `lib/build.mjs:416`. Pass the untouched base
model into `augmentSiteModel` at `lib/build.mjs:385`, and assert both its input and return have
...
`lib/emit/digest.mjs:25`, page foot at `lib/emit/page.mjs:397`, and README sample line at
...
`lib/emit/report.mjs:47`, `lib/emit/post.mjs:16`, and `lib/emit/page.mjs:375`. (D6, D25, D55)
...
stdout branch at `lib/build.mjs:468` byte for byte, including its optional goals and archive suffixes:
...
This replaces the current AGENTS wording at `AGENTS.md:40` and README wording at `README.md:292`
