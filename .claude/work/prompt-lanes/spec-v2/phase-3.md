...
`week.start` created at `lib/build.mjs:253`, identical on every row the build emits. Do not accept an
...
The current base-model assembly is at `lib/build.mjs:285`; site augmentation is at
`lib/build.mjs:355`; page construction and redaction are at `lib/build.mjs:385` and
`lib/build.mjs:386`; the markdown redaction call is at `lib/build.mjs:405`. Before attachment,
...
work-only span site at `lib/emit/page.mjs:352`. (D8, D15, D16, D25, D35, D52, D55, D62)
...
`lib/validate.mjs:165`; do not widen work-item validation. Validate the R3 authored shape,
