...
1. **`init`** *(input: none; output: `honestweek.config.json`)*
   ...
   `--yes` is required here: bare `init` asks two confirmations at the terminal, and your shell has no TTY, so it exits `2` telling you to pass `--yes`. Accepting the inferred defaults is what `--yes` does; it still refuses to overwrite an existing config.
